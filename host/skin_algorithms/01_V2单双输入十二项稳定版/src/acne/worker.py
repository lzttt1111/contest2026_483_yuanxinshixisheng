...
from aisia_contracts.algorithms.acne.v2 import SCHEMA_VERSION as _ACNE_V2_SV
...
def _load_scoring_summary(pipeline_result: Mapping[str, Any]) -> dict[str, Any] | None:
    path = pipeline_result.get("summary_path")
    ...
    try:
        ...
        logger.exception("读取痤疮评分 summary 失败")
        return None
    return dict(value) if isinstance(value, dict) else None


def _build_acne_scoring_input(
    task: object,
    *,
    record_id: str,
    image_sha256: str,
    pipeline_result: Mapping[str, Any],
    medical_v2_additions: Mapping[str, Any],
    input_quality_gate: Mapping[str, Any] | None = None,
) -> dict[str, Any] | None:
    """检测结束、清理前生成痤疮 v2 的版本化评分输入。"""
    summary = _load_scoring_summary(pipeline_result)
    if summary is None:
        return None
    medical = medical_v2_additions.get("medical_metrics_v2")
    profile = capture_profile_from_environment().value
    return build_scoring_input(
        algorithm_name="acne",
        detection_schema_version=_ACNE_V2_SV,
        detection_impl_version=_ACNE_V2_SV,
        report_id=record_id,
        detection_attempt_id=_detection_attempt_id(task, record_id),
        source_image_sha256=image_sha256,
        capture_profile=profile,
        input_route=_input_route(profile),
        runtime={
            "summary": summary,
            "medical_metrics_v2": medical if isinstance(medical, Mapping) else None,
        },
        quality={},
...
def analyze_image(self, task_id, oss_key, oss_result_prefix=None, algorithms=None, *, capture_images=None):
    ...
    with tempfile.TemporaryDirectory(prefix=f"{_SERVICE_NAME}_{record_id}_") as tmpdir:
        ...
        try:
            pipeline_result = run_task_pipeline("acne", _get_pipeline(), input_path, algorithms)
    ...
    if FORMAL_FAST_TOKEN in (algorithms or []):
        scoring_input = _build_acne_scoring_input(
            self,
            record_id=record_id,
            image_sha256=image_sha256,
            pipeline_result=pipeline_result,
            medical_v2_additions=medical_v2_additions,
            input_quality_gate=compute_input_quality_gate(img_bytes),
        )
        if scoring_input is not None:
            response["_scoring_input"] = scoring_input
    return response
...
def _build_acne_v2_response(legacy_response: dict) -> dict:
    ...
    scoring_input = legacy_response.get("_scoring_input")
    raw_result_v2 = {
        "overlay": overlay,
        "metrics": {
            "疑似痤疮数量": counts,
...
@with_capture_images("acne")
def analyze_image_v2(self, task_id, oss_key, oss_result_prefix=None, algorithms=None, *, capture_images=None):
