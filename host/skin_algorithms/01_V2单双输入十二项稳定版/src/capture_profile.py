...
_REQUEST_PROFILE: ContextVar[CaptureProfile | None] = ContextVar("capture_profile", default=None)


@contextmanager
def use_capture_profile(profile: CaptureProfile):
...
def current_capture_profile(default: CaptureProfile) -> CaptureProfile:
    return _REQUEST_PROFILE.get() or default


def capture_profile_from_environment() -> CaptureProfile:
    request_profile = _REQUEST_PROFILE.get()
