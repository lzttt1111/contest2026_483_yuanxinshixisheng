...
class ClinicFourLightProvider(RouteModalityProvider):
    ...
    def run_prepared(
        ...
        *,
        ...
        allowed = {"redness", "brown", "purple", "surface_gloss", "vascular"}
        selected = allowed if algorithms is None else set(algorithms)
        if not selected or not selected <= allowed:
            raise ValueError("unsupported clinic algorithm selection")
