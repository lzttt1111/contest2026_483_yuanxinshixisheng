...
def build_institution_complete_document(
    ...
    for item_id, runtime_item in runtime_items.items():
        ...
        if item_id == "contour_firmness":
            # Retain the established institution Word aliases alongside the
            # measured geometry required by the machine-score formulas.
            for name, value in sanitize_public_document(_read_json(public_path)).items():
