...
def validate_csv_matches_document(
    ...
    # Existing pore reports label pixel-count areas as 像素². Normalize only
    # these four known area headers, retaining the full numeric comparison.
    area_aliases = {
        name.replace("（像素）", "（像素²）"): name
        for name in ("核心-P50单体面积（像素）", "核心-P90单体面积（像素）",
                     "辅助-特征总面积（像素）", "辅助-最大单体面积（像素）")
    }
    mapped_columns = [area_aliases.get(name, name) for name in actual_columns]
    if len(set(mapped_columns)) != len(mapped_columns):
        raise ValueError("医学 V2 CSV 存在重复的面积单位别名列")
    actual_columns = mapped_columns
    actual_rows = [{area_aliases.get(k, k): v for k, v in row.items()} for row in actual_rows]
