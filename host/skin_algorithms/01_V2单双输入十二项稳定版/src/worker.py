...
后端对接契约（ai-skin-backend → Celery）：
  ...
  - 正式单算法成功返回六字段：record_id / status / schema_version /
    meta_data / raw_result / debug_info。raw_result按WORKER_CONTRACT.md和严格Pydantic
...
from aisia_contracts.algorithms.redness.v3 import SCHEMA_VERSION as _REDNESS_SV
from aisia_contracts.algorithms.spots.v3 import SCHEMA_VERSION as _SPOTS_SV
from aisia_contracts.algorithms.brown.v3 import SCHEMA_VERSION as _BROWN_SV
from aisia_contracts.algorithms.texture.v3 import SCHEMA_VERSION as _TEXTURE_SV
from aisia_contracts.algorithms.pores.v3 import SCHEMA_VERSION as _PORES_SV
from aisia_contracts.algorithms.purple.v2 import SCHEMA_VERSION as _PURPLE_SV
from aisia_contracts.algorithms.surface_gloss.v1 import (
    SCHEMA_VERSION as _SURFACE_GLOSS_SV,
)
from aisia_contracts.algorithms.vascular.v1 import SCHEMA_VERSION as _VASCULAR_SV
from aisia_contracts.algorithms.contour_firmness.v1 import (
    SCHEMA_VERSION as _CONTOUR_FIRMNESS_SV,
)
...
_SCHEMA_VERSIONS = {
    ...
    "pores": _PORES_SV,
    "purple": _PURPLE_SV,
    "surface_gloss": _SURFACE_GLOSS_SV,
    "vascular": _VASCULAR_SV,
    "contour_firmness": _CONTOUR_FIRMNESS_SV,
}
...
from src.scoring_input import build_scoring_input, compute_input_quality_gate

_SCORING_FAST_FIVE = frozenset({"redness", "spots", "brown", "texture", "pores"})
_SCORING_FULL_METRICS = frozenset(
    {"purple", "surface_gloss", "vascular", "contour_firmness"}
)

from src.detection_runtime.cloud_task import with_capture_images, download_task_input, close_capture_runtime
from src.detection_runtime.cloud_inference import run_task_pipeline
from src.capture_profile import current_capture_profile
...
WORKER_TARGETS: dict[str, dict[str, object]] = {
    ...
    # 独立 CPU 聚合目标：与检测 Worker 同源 broker，但 app 实例与队列独立；
    # 不产出检测结果，故 results 为空，不进入十二项结果清单。
    # 常量 SSOT 在 src.summary；此处字面量由 tests 保持与之一致。
    "summary": {"app": "src.summary.worker:celery_app", "queue": "summary", "task": "dermavision.aggregate_summary", "results": (), "concurrency": 2},
}
...
def _load_json_mapping(path: object) -> dict[str, Any] | None:
    ...
    try:
        ...
        logger.warning("读取评分输入运行时 JSON 失败: %s", path)
        return None
    return dict(value) if isinstance(value, dict) else None


def _load_medical_v2_document(pipeline_result: dict, algorithm: str) -> dict[str, Any] | None:
    """加载医学 V2 旁路文档；与上传成功无关，生成成功即纳入。"""
    spec = (pipeline_result.get("medical_v2_results") or {}).get(algorithm)
    if not isinstance(spec, dict):
        return None
    json_path = spec.get("json")
    ...
    if not json_path or not csv_path:
        return None
    try:
        return load_and_validate(json_path, csv_path)
    except Exception:
        logger.exception("Record 医学 V2 文档读取失败 algorithm=%s", algorithm)
        return None


def _scoring_quality(pipeline_result: dict) -> dict[str, Any]:
...
def _detection_attempt_id(task: object, record_id: str) -> str:
    request = getattr(task, "request", None)
    attempt = getattr(request, "id", None) if request is not None else None
    return str(attempt or record_id)


def _input_route(profile: str) -> str:
    return "consumer_queue" if profile == "consumer" else "institution_queue"


def _build_root_scoring_input(
    algorithm: str,
    *,
    record_id: str,
    attempt_id: str,
    image_sha256: str,
    pipeline_result: dict,
    inline_metrics: Mapping[str, Any],
    local_results: Mapping[str, Any],
    input_quality_gate: Mapping[str, Any] | None = None,
) -> dict[str, Any]:
    """在清理中间文件前，把单算法运行时结果投影成版本化评分输入。"""
    runtime: dict[str, Any] = {"metrics": inline_metrics.get(f"{algorithm}_metrics")}
    clinic_evidence = pipeline_result.get("scoring_detector_results")
    if isinstance(clinic_evidence, Mapping):
        runtime["detector_results"] = clinic_evidence
    if algorithm in _SCORING_FULL_METRICS:
        runtime["full_metrics"] = _load_json_mapping(
            local_results.get(f"{algorithm}_full_metrics")
        )
    if algorithm in _SCORING_FAST_FIVE:
        runtime["medical_metrics_v2"] = (
            _load_medical_v2_document(pipeline_result, algorithm)
            if settings.enable_medical_metrics_v2
            else None
        )
    profile = current_capture_profile(settings.capture_profile).value
    return build_scoring_input(
        algorithm_name=algorithm,
        detection_schema_version=_SCHEMA_VERSIONS[algorithm],
        detection_impl_version="1",
        report_id=record_id,
        detection_attempt_id=attempt_id,
        source_image_sha256=image_sha256,
        capture_profile=profile,
        input_route=_input_route(profile),
        runtime=runtime,
        quality=_scoring_quality(pipeline_result),
...
def _build_envelope_response(
    ...
    """按WORKER_CONTRACT.md构建单算法六字段信封，算法内部结果保持不变。"""
    algo = algorithms[0]
    if algo == "purple":
        algorithm_result = {
    ...
    if "scoring_input" in raw_result:
        algorithm_result["scoring_input"] = raw_result["scoring_input"]
    response = {
        "record_id": record_id,
        "status": "success",
...
@with_capture_images("dermavision")
def analyze_image(self, task_id, oss_key, oss_result_prefix=None, algorithms=None, *, capture_images=None):
    """皮肤检测异步任务。
    ...
    try:
        img_bytes = download_task_input(oss_key, download_via_internal_api)
    ...
    with tempfile.TemporaryDirectory(prefix=f"dermavision_{record_component}_") as tmpdir:
        ...
        try:
            ...
            pipeline_result = run_task_pipeline("dermavision", pipeline, input_path, algorithms)
        ...
        if len(algorithms) == 1 and algorithms[0] in _ENVELOPE_ALGOS:
            # 同源门禁：对下载原图 bytes 真实调用 V0.1.1 evaluate_image_path，
            # 在清理中间文件前生成并随信封持久化；不可计算时诚实为 None。
            input_quality_gate = compute_input_quality_gate(img_bytes)
            medical_v2_additions = _build_medical_v2_additions(
                pipeline_result,
                algorithms[0],
                record_component,
            )
            raw_result["scoring_input"] = _build_root_scoring_input(
                algorithms[0],
                record_id=str(record_id),
                attempt_id=_detection_attempt_id(self, str(record_id)),
    ...
    metadata["execution_mode"] = (
        execution_mode_for_profile(current_capture_profile(settings.capture_profile), "single_algorithm")
        if len(algorithms) == 1
