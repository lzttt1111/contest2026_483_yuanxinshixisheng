...
from aisia_contracts.algorithms.wrinkle.v2 import SCHEMA_VERSION as _WRINKLE_SV
from src.capture_profile import capture_profile_from_environment
...
def _build_wrinkle_scoring_input(
    task: object,
    *,
    record_id: str,
    image_sha256: str,
    ...
    return build_scoring_input(
        algorithm_name="wrinkle",
        detection_schema_version=_WRINKLE_SV,
        detection_impl_version="1",
        report_id=str(record_id),
        detection_attempt_id=_detection_attempt_id(task, str(record_id)),
...
def analyze_image(self, task_id, oss_key, oss_result_prefix=None, algorithms=None, *, capture_images=None):
    ...
    with tempfile.TemporaryDirectory(prefix=f"{_SERVICE_NAME}_{record_id}_") as tmpdir:
        ...
        try:
            ...
            pipeline_result = run_task_pipeline("wrinkle", pipeline, input_path, algorithms)
        ...
        scoring_metadata = dict(metadata) if isinstance(metadata, Mapping) else {}
        summary_path = local_results.get("summary_json")
        if summary_path and Path(summary_path).is_file():
            try:
                full_summary = json.loads(Path(summary_path).read_text(encoding="utf-8"))
                if isinstance(full_summary, dict):
                    scoring_metadata["summary"] = full_summary
            except (OSError, ValueError):
                logger.exception("皱纹完整评分证据不可读取，保留已有证据并标记缺失项")
        raw_result["scoring_input"] = _build_wrinkle_scoring_input(
            self,
            record_id=str(record_id),
    ...
    response = {
        ...
        "raw_result": {
            ...
            "scoring_input": raw_result.get("scoring_input"),
            **medical_v2_additions,
        },
        "debug_info": {
