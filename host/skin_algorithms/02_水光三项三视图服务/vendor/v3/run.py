...
""",
    ...
    parser.add_argument(
        "--capture-alias",
        action="append",
        help="仅运行清单中指定的四光源采集编号；可重复，默认运行manifest全部样本",
    )
    parser.add_argument(
        "--output-dir",
        "--output",
    ...
    parser.add_argument(
        ...
        choices=("v2", "v3"),
        default="v2",
        help="医生指标规范；v3为九月独立候选，v2保持原行为",
    )
    parser.add_argument("--captured-at", default=None, help="真实采集时间，ISO8601带时区；不填则不推断")
    parser.add_argument("--v3-reference", type=Path, default=None, help="显式、同Profile的V3参考配置")
    parser.add_argument("--v3-score-reference",type=Path,default=None,help="阶段一统一评分参考；只允许正式用途配置")
    parser.add_argument(
        "--capture-profile",
...
def _run_clinic_mode(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        for capture in captures:
            ...
            if getattr(args,"clinical_spec","v2")=="v3":
                from src.doctor_v3.stage1_resume import matches,mark_complete
                stage_signature={"capture_profile":"institution","v3_versions":args.v3_versions,
                    "input_sha256":{c.role:c.sha256 for c in capture.channels},
                    "subject_id":args.report_subject_id or capture.capture_alias,
                    "v3_output_profile":args.output_profile,"v3_pdf_requested":bool(args.generate_medical_report)}
                if matches(output_root/capture.capture_alias,stage_signature):
                    completed+=1
                    continue
                from src.doctor_v3.stage1_retry import prepare_retry
                prepare_retry(output_root/capture.capture_alias)
            result = orchestrator.analyze_capture(capture)
            source_root = orchestrator.run_root / "repeat_1" / capture.capture_alias
            if getattr(args,"clinical_spec","v2")=="v3" and args.output_profile=="scoring":
                from src.doctor_v3.stage1_pipeline import from_result,save,configured_reference
                from src.doctor_v3.stage1_config import measurement_config
                from src.doctor_v3.stage1_storage import append_sample
                if result.get("状态")!="success":raise ValueError("incomplete capture cannot be a scoring sample")
                payload,_,_,_=from_result(source_root,subject_id=stage_signature["subject_id"],profile="institution",
                    input_sha256=stage_signature["input_sha256"],runtime_items=result["十二项结果"],
                    thresholds=measurement_config(args.v3_calibration),reference=configured_reference(),
                    registration={c.role:c.registration_status=="PASSED" and all(c.cross_channel_authorized.get(k) is True for k in ("fusion","overlap","co_location")) for c in capture.channels})
                destination=output_root/capture.capture_alias
                payload["batch_signature"]=stage_signature
                save(destination/"v3_scoring",payload)
                append_sample(output_root/"v3_scoring.institution.jsonl",payload)
                mark_complete(destination,stage_signature)
                completed+=1
                if not args.keep_runtime:shutil.rmtree(source_root,ignore_errors=True)
                continue
            ...
            if args.generate_medical_report and getattr(args, "clinical_spec", "v2") == "v2":
            ...
            if getattr(args, "clinical_spec", "v2") == "v3":
                from src.doctor_v3.delivery import export_v3
                export_v3(destination, source_root, result["十二项结果"],
                          subject_id=args.report_subject_id or capture.capture_alias,
                          profile="institution", captured_at=args.captured_at,
                          generate_pdf=args.generate_medical_report,
                          reference_path=args.v3_reference,stage1=True,input_hashes=stage_signature["input_sha256"],
                          registration={channel.role: (
                              channel.registration_status == "PASSED" and
                              all(channel.cross_channel_authorized.get(k) is True for k in ("fusion","overlap","co_location"))
                          ) for channel in capture.channels})
                mark_complete(destination,stage_signature,
                    [p.name for p in destination.glob("*.docx")] if args.generate_medical_report else ())
            completed += 1
            print(
...
def main(argv: list[str] | None = None) -> int:
    ...
    os.environ["DERMAVISION_CLINICAL_SPEC"] = args.clinical_spec
    if args.clinical_spec == "v3":
        from src.doctor_v3.stage1_runtime import configure_runtime, require_output
        from src.doctor_v3.stage1_schema import VERSIONS
        configure_runtime()
        require_output(args.output_dir)
        if args.qc_review_dir is not None:require_output(args.qc_review_dir)
        if args.runtime_dir is not None:require_output(args.runtime_dir)
        from src.doctor_v3.calibration import resolve_reference_inputs
        try:
            if args.v3_reference is not None:require_output(args.v3_reference)
            if args.v3_score_reference is not None:require_output(args.v3_score_reference)
            references=resolve_reference_inputs(args.v3_reference,args.v3_score_reference,args.capture_profile)
            args.v3_reference=references['reference_path']
            args.v3_score_reference=references['score_reference_path']
            args.v3_calibration=references['calibration']
            args.v3_reference_hash=references['reference_file_sha256']
        except ValueError as exc:
            parser.error(str(exc))
        if args.algorithms != ["all"] and not (
            args.output_profile == "scoring"
            and tuple(args.algorithms) == ("pores", "spots", "surface_gloss")
        ):
            parser.error("V3正式报告要求完整十二项；水光scoring仅允许pores/spots及其brown/uv支持证据/surface_gloss")
        if args.slots != 1:
            parser.error("V3阶段一仅允许单槽；并行批跑需后续独立验收")
        args.v3_versions = VERSIONS
        os.environ.pop("DERMAVISION_V3_SCORE_REFERENCE",None)
        if args.v3_score_reference is not None:
            require_output(args.v3_score_reference)
            os.environ["DERMAVISION_V3_SCORE_REFERENCE"]=str(args.v3_score_reference.resolve())
            from src.doctor_v3.stage1_pipeline import configured_reference
            configured_reference()
        from src.doctor_v3.stage1_pipeline import configured_reference
        from src.doctor_v3.stage1_schema import version_identity
        from src.doctor_v3.stage1_config import measurement_config
        args.v3_versions=version_identity(measurement_config(args.v3_calibration),configured_reference())
        if args.runtime_dir is None:
            args.runtime_dir = Path(__file__).resolve().parent.parent / "05_V3验收结果" / "runtime"
        if args.captured_at:
            try:
                if datetime.fromisoformat(args.captured_at).tzinfo is None:
                    raise ValueError("missing timezone")
            except ValueError:
                parser.error("--captured-at必须是带时区的ISO8601真实采集时间")
    if args.slots == 2:
    ...
    with (
        ...
        try:
            ...
            for global_index, image in enumerate(
                ...
                if args.clinical_spec == "v3":
                    signature["clinical_spec"] = "doctor-v3-stage1"
                    signature["v3_versions"] = args.v3_versions
                    signature["v3_output_profile"] = args.output_profile
                    signature["v3_pdf_requested"] = args.generate_medical_report
                    signature["v3_reference"] = args.v3_reference_hash
                    signature["v3_captured_at"] = args.captured_at
                    from src.doctor_v3.stage1_identity import augment_consumer_signature
                    signature = augment_consumer_signature(signature, image, args.report_subject_id)
                if (
                    args.output_profile == "review"
                ...
                if args.clinical_spec == "v3":
                    result_index = expected_output / "v3_scoring" / "manifest.json"
                if should_skip_terminal(
                    signature_key(signature),
                    completed,
                    output_profile=args.output_profile,
                    skip_recorded_failures=args.skip_recorded_failures,
                    result_index_exists=(result_matches_signature(expected_output,signature) if args.clinical_spec=="v3" else result_index.is_file()),
                ):
                    skipped_count += 1
                    continue
                ...
                try:
                    try:
                        ...
                        if (
                            ...
                            if args.generate_medical_report and args.clinical_spec == "v2" and is_full_twelve_selection(selected_algorithms):
                            ...
                            if is_full_twelve_selection(selected_algorithms):
                                ...
                                if args.clinical_spec == "v3":
                                    from src.doctor_v3.delivery import export_v3
                                    export_v3(exported, source_root, result["十二项结果"],
                                              subject_id=signature["subject_id"], input_hashes=signature["input_sha256"],
                                              profile=args.capture_profile, captured_at=args.captured_at,
                                              generate_pdf=args.generate_medical_report,reference_path=args.v3_reference,stage1=True)
                            ...
                            if args.clinical_spec == "v3":
                                from src.doctor_v3.stage1_delivery import export_scoring
                                from src.doctor_v3.stage1_storage import append_sample
                                from src.doctor_v3.calibration import load_reference
                                from src.doctor_v3.stage1_config import measurement_config
                                calibration=load_reference(args.v3_reference,args.capture_profile)
                                v3_payload=export_scoring(source_root,expected_output,
                                    subject_id=signature["subject_id"],
                                    profile=args.capture_profile,image=image,runtime_items=result["十二项结果"],
                                    thresholds=measurement_config(calibration),batch_signature=signature)
                                append_sample(state_dir / ("v3_scoring."+worker_id+".jsonl"),v3_payload)
                            record.update(
                                status="success",
                                output_dir=(expected_output if args.clinical_spec=="v3" else scoring_path).relative_to(output_root).as_posix(),
                            )
                        if args.clinical_spec=="v3" and record.get("status")=="success":
                            from src.doctor_v3.stage1_resume import mark_complete
                            mark_complete(expected_output,signature,
                                [p.name for p in expected_output.glob("*.docx")] if args.generate_medical_report else ())
                        if not args.keep_runtime:
                            shutil.rmtree(source_root, ignore_errors=True)
                        ...
                        if export_lease is not None:
                            export_lease.close()
                            export_lease = None
                        if args.clinical_spec=="v3":record["status"]="failed"
                        else:record.setdefault("status", "failed")
                        record.setdefault(
