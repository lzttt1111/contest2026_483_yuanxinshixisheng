...
class AcneAnalysisRunner:
    ...
    def run(
        ...
        try:
            ...
            if enabled():
                save(output_dir, "acne",
                     {"valid": result.skin_mask, "landmarks": result.landmarks_xy,
                      "image": result.standardized_bgr},
                     {"instances": detection.get("detections", []),
                      "quality_status": "PASS" if detection.get("status") == "ok" else "REJECT",
                      "source_kind": "rgb_measured"})
            summary = {
                "status": "ok",
                "profile": {
