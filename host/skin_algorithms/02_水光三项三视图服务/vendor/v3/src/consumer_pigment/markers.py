...
# Module-specific high-contrast variants of the approved redness marker.
...
def consolidate_existing_marker_mask(
    ...
    *,
    ...
    for label in range(1, labels_count):
        ...
        ring = (
            ((yy - peak_y) ** 2 + (xx - peak_x) ** 2 >= 8 ** 2)
            & ((yy - peak_y) ** 2 + (xx - peak_x) ** 2 <= 16 ** 2)
            & valid[y0:y1, x0:x1]
            & ~exclusion[y0:y1, x0:x1]
        ...
        if background.size:
            ...
            required_prominence = minimum_local_prominence_z / (
                1.0
                + large_component_prominence_relief
                * math.log2(area_scale)
            )
        ...
        radius = int(
            np.clip(
                round(marker_radius_scale * np.sqrt(source_area / np.pi)),
                2,
                7,
            )
        )
        proposed.append(
            ExistingMarkerFinding(
                centroid=(int(peak_x), int(peak_y)),
    ...
    proposed.sort(
        key=lambda item: (
            ...
            * min(math.log2(1.0 + item.source_area_px / 25.0), 3.0)
        ),
        reverse=True,
    )
    ...
    for finding in proposed:
        ...
        if any(
            (x - other.centroid[0]) ** 2 + (y - other.centroid[1]) ** 2
            < (
                max(
                    minimum_distance_px,
                    (
                        finding.marker_radius_px
                        + other.marker_radius_px
                        + minimum_marker_edge_gap_px
                    )
                    if minimum_marker_edge_gap_px > 0
                    else minimum_distance_px,
                ) ** 2
            )
...
def render_compact_marker_mask(
    ...
    if np.any(marker_pixels):
        ...
        output[marker_pixels] = np.clip(
            (1.0 - style.fill_alpha)
            * output[marker_pixels].astype(np.float32)
            + style.fill_alpha * fill,
            0.0,
            255.0,
        ).astype(np.uint8)
    contours, _ = cv2.findContours(
        (marker_pixels.astype(np.uint8) * 255),
        cv2.RETR_EXTERNAL,
