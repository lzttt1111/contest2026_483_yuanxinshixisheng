...
def run_vascular_v3(
    ...
    if enabled() and detector is None:
        active_detector.include_forehead = True
    ...
    if enabled():
        save(root, "vascular",
             {"valid": result.valid_mask, "landmarks": request.measurement.landmarks,
              "instances": result.vascular_mask, "skeleton": result.skeleton_mask,
              "score": result.hemoglobin_response},
             {"instances": list(result.instances), "quality_status": request.measurement.quality_status,
