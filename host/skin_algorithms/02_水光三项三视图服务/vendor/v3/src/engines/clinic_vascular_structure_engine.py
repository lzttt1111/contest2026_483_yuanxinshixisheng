...
class ClinicVascularStructureAnalyzer:
    ...
    def detect(
        ...
        *,
        ...
        if self.include_forehead:
            points = np.asarray(cp.landmarks)
            self._v3_forehead_end = float(np.max(points[np.asarray(LEFT_EYEBROW+RIGHT_EYEBROW),1])) + 12
    ...
    def _clinic_exclusion(self, image: np.ndarray, face: np.ndarray, cp: Any) -> np.ndarray:
        ...
        if brow_points.size and not self.include_forehead:
            forehead_end = int(
                np.clip(
    ...
    def _filter_components(
        ...
        for label in range(1, count):
            ...
            region = self._assign_clinical_region(cx, cy, valid)
    ...
    def _region_metrics(self, valid: np.ndarray, instances: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
        ...
        left = nx < 0.5
        masks = {
            "forehead": (ys < self._v3_forehead_end) if self.include_forehead else np.zeros_like(ny, dtype=bool),
            "nose_alar_nasal_side": (nx >= 0.38) & (nx <= 0.62) & (ny >= 0.30) & (ny <= 0.68),
        }
        masks["nose_alar_nasal_side"] &= ~masks["forehead"]
    ...
    def _assign_clinical_region(self, x: float, y: float, valid: np.ndarray) -> str:
        if self.include_forehead and y < self._v3_forehead_end:
            return "forehead"
        return self._assign_region(x,y,valid)
