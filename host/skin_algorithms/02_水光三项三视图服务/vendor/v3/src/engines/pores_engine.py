...
class PoresAnalyzer:
    ...
    def detect_pores(self, preprocess_result: PreprocessResultV2) -> PoresResult:
        ...
        nasolabial_shadow = build_nasolabial_shadow_mask(
            ...
            output_margin_px=1 if os.environ.get("SHUIGUANG_PREPROCESS_MODE")=="semantic_hair_candidate_v1" else Config.NASOLABIAL_SHADOW_MARGIN_PX,
            restrict_frangi_to_corridor=True,
            require_thin_component=os.environ.get("SHUIGUANG_PREPROCESS_MODE")=="semantic_hair_candidate_v1",
        )
        # This remains a deliberately unassessed geometric strip, not a
        # successful beard segmentation. The short-strand candidate failed QA.
        moustache_exclusion = build_moustache_feature_exclusion_mask(
            image.shape[:2],preprocess_result.landmarks,regions.analysis_mask,margin_px=12)
