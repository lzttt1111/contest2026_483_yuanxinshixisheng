...
from src.engines.uv_spots_engine import UVSpotsAnalyzer, MEASUREMENT_SUPPORT_DEFINITION
...
class PurpleAnalysisEngine:
    ...
    def analyze(
        ...
        *,
        ...
        uv_measurement_support = uv_spots_result.measurement_support_mask
        ...
        match self.capture_profile:
            ...
            case CaptureProfile.CONSUMER:
                ...
                if uv_measurement_support is not None:
                    uv_measurement_support = cv2.bitwise_and(
                        _binary_mask(uv_measurement_support), cv2.bitwise_not(formal_exclusion)
                    )
        ...
        result = PurpleAnalysisResult(
            ...
            measurement_support_mask=uv_measurement_support,
        )
    ...
    def process_preprocess_result(
        ...
        if enabled():
            for project, mask, score, instances in (
                ("uv_spots", result.uv_spots_mask, result.uv_spots_score_map, result.uv_spots_instances),
                ("porphyrin", result.porphyrin_mask, result.porphyrin_score_map, result.porphyrin_locations),
            ):
                save(os.path.dirname(self.last_paths["porphyrin_overlay"]), project,
                     {"valid": result.analysis_mask, "landmarks": result.landmarks,
                      "instances": mask, "score": score,
                      **({"continuous": result.measurement_support_mask} if project == "uv_spots" else {})},
                     {"instances": instances, "quality_status": result.quality_status,
                      "source_kind": "rgb_proxy",
                      **({"continuous_definition": MEASUREMENT_SUPPORT_DEFINITION,
                          "continuous_source": "UVSpotsAnalyzer.accepted_components_before_display",
                          "continuous_exclusion": "instance_and_consumer_specular",
                          "pixel_source_role": "RGB_M"} if project == "uv_spots" else {})})
