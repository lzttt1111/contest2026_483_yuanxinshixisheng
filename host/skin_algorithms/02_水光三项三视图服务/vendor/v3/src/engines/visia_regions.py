...
def build_nasolabial_shadow_mask(
    ...
    *,
    ...
    require_thin_component: bool = False,
) -> np.ndarray:
    ...
    candidate = cv2.morphologyEx(
        candidate,
        cv2.MORPH_CLOSE,
        cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3) if require_thin_component else (7, 7)),
    )
    ...
    for label in range(1, component_count):
        ...
        if area < 8 or not np.any(component & (path_support > 0)):
            continue
        if require_thin_component:
            ys,xs=np.where(component)
            _,(width,height),_=cv2.minAreaRect(np.column_stack((xs,ys)).astype(np.float32))
            if (max(width,height)+1)/(min(width,height)+1)<3.0:
                continue
            thickness=cv2.distanceTransform(component.astype(np.uint8),cv2.DIST_L2,5)
            if float(thickness.max())>4.0:
                continue
