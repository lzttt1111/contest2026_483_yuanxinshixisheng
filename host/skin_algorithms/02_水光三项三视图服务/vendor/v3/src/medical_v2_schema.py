...
INCREMENTAL_OVERALL_METRICS: dict[str, frozenset[str]] = {
    "redness": frozenset({"diffuse_red_area_px", "diffuse_red_area_ratio", "redness_continuity", "redness_boundary_gradient", "diffuse_redness_uniformity", "feature_density_per_100k_skin_px", "p50_instance_area_px", "p90_instance_area_px", "max_instance_area_px", "mean_instance_intensity", "p50_instance_intensity", "p90_instance_intensity", "central_face_feature_concentration", "continuous_region_count", "max_continuous_region_area_px"}),
    "spots": frozenset({"feature_density_per_100k_skin_px", "total_feature_area_px", "p90_instance_area_px", "max_instance_area_px", "p50_delta_e", "p90_delta_e", "punctate_spot_count", "punctate_spot_area_ratio", "patch_spot_count", "patch_spot_area_ratio", "confluent_candidate_count", "confluent_candidate_area_ratio", "median_boundary_sharpness", "median_color_uniformity", "primary_concentration_region"}),
    "brown": frozenset({"feature_density_per_100k_skin_px", "total_feature_area_px", "p50_instance_area_px", "p90_instance_area_px", "max_instance_area_px", "continuous_region_count", "max_continuous_region_area_px", "brown_spot_area_ratio", "continuous_brown_coverage_area_px", "continuous_brown_coverage_ratio", "mean_instance_intensity", "p50_instance_intensity", "p90_instance_intensity", "mean_intensity", "p50_intensity", "p90_intensity", "p95_intensity", "high_intensity_area_ratio", "brown_pigmentation_burden", "punctate_brown_target_count", "patch_brown_target_count", "punctate_target_count", "punctate_target_area_ratio", "patch_target_count", "patch_target_area_ratio", "primary_concentration_region"}),
    "texture": frozenset({"feature_density_per_100k_skin_px", "total_feature_area_px", "feature_area_ratio", "p50_instance_area_px", "p90_instance_area_px", "max_instance_area_px", "mean_instance_intensity", "p50_instance_intensity", "p90_instance_intensity", "mean_intensity", "p50_intensity", "p90_intensity", "p95_intensity", "raised_like_ratio", "depressed_like_ratio", "raised_like_area_ratio", "depressed_like_area_ratio", "cluster_region_count", "max_cluster_feature_count", "max_cluster_area_px", "primary_issue_type", "primary_concentration_region"}),
    "pores": frozenset({"feature_density_per_100k_skin_px", "total_feature_area_px", "feature_area_ratio", "p50_instance_area_px", "p90_instance_area_px", "max_instance_area_px", "mean_center_ring_contrast", "p50_center_ring_contrast", "p90_center_ring_contrast", "p50_equivalent_diameter_px", "p90_equivalent_diameter_px", "p50_circularity", "p50_aspect_ratio", "low_circularity_pore_ratio", "elliptical_pore_ratio", "elongated_pore_ratio", "primary_concentration_region"}),
    "purple_uv_spots": frozenset({"punctate_target_count", "punctate_target_area_ratio", "patch_target_count", "patch_target_area_ratio", "continuous_abnormal_area_px", "continuous_abnormal_area_ratio", "continuous_region_count", "max_continuous_region_area_px", "cluster_region_count", "max_cluster_area_px", "high_intensity_target_ratio"}),
    "purple_porphyrin": frozenset({"punctate_target_count", "punctate_target_area_ratio", "patch_target_count", "patch_target_area_ratio", "continuous_abnormal_area_px", "continuous_abnormal_area_ratio", "continuous_region_count", "max_continuous_region_area_px", "cluster_region_count", "max_cluster_area_px", "high_intensity_target_ratio"}),
}
INCREMENTAL_REGION_METRICS = dict(INCREMENTAL_OVERALL_METRICS)
INCREMENTAL_LATERALITY_METRICS = {
    "redness": frozenset({"image_cheek_density_difference", "image_cheek_area_ratio_difference", "image_cheek_p90_intensity_difference"}),
    "spots": frozenset({"image_cheek_density_difference", "image_cheek_area_ratio_difference"}),
    "brown": frozenset({"image_cheek_density_difference", "image_cheek_area_ratio_difference", "image_cheek_p90_intensity_difference"}),
    "texture": frozenset({"image_cheek_density_difference", "image_cheek_area_ratio_difference"}),
    "pores": frozenset({"image_cheek_density_difference", "image_cheek_area_ratio_difference"}),
}
_PROJECT_VALUE_TO_KEY = {"红区": "redness", "可见红区": "redness", "可见斑点": "spots", "斑点": "spots", "普通RGB可见斑点": "spots", "棕区": "brown", "棕色斑": "brown", "棕区（普通RGB综合色素代理）": "brown", "纹理": "texture", "二维可见表面纹理代理": "texture", "毛孔": "pores", "可见毛孔负担": "pores", "紫区（紫外线色斑与紫质）": "purple"}
...
def _prune_regions(rows: Any, project: str) -> list[dict[str, Any]]:
    ...
    for row in rows:
        ...
        if any(groups for groups in metric_groups.values()): output.append(selected)
    return output
...
def to_incremental_document(document: dict[str, Any], project: str | None = None) -> dict[str, Any]:
    ...
    key = project or _PROJECT_VALUE_TO_KEY.get(str(public.get("project") or ""), str(public.get("project") or ""))
    overall = public.get("overall_metrics", {})
    if key == "purple":
        incremental_overall = {sub: _prune_metric_groups(overall.get(sub, {}), INCREMENTAL_OVERALL_METRICS[f"purple_{sub}"]) for sub in ("uv_spots", "porphyrin")}
        incremental_regions = []
        for index, group in enumerate(public.get("region_metrics", [])):
            sub = "uv_spots" if index == 0 else "porphyrin"
            regions = _prune_regions(group.get("region_metrics", []), f"purple_{sub}") if isinstance(group, dict) else []
            if regions: incremental_regions.append({"subproject": sub, "regions": regions})
        left_right = {}
    else:
        allowed = INCREMENTAL_OVERALL_METRICS.get(key)
        if allowed is None: raise ValueError(f"未知医学 V2 项目: {key!r}")
        ...
        left_right = {name: value for name, value in raw_laterality.items() if name in allowed_laterality} if isinstance(raw_laterality, dict) else {}
    result = {"metrics_version": public.get("metrics_version", "medical_metrics_v2_20260728"), "scoring_status": public.get("scoring_status", "uncalibrated"), "units": public.get("imaging_and_units", {}), "overall_metrics": incremental_overall, "region_metrics": incremental_regions, "left_right_comparison": left_right, "medical_limitations": list(public.get("medical_limitations") or [])}
