...
class MaskShapeError(ValueError):
    ...
    def __str__(self) -> str:
        return (
            f"{self.label} mask shape mismatch: "
...
def expand_common_hair_masks(
    ...
    hair = cv2.dilate(
        (np.asarray(hair_mask) > 0).astype(np.uint8) * 255,
        kernel,
    )
    facial = cv2.dilate(
        (np.asarray(facial_hair_mask) > 0).astype(np.uint8) * 255,
        kernel,
    )
    return hair, facial
...
class AnalysisMaskBundle:
        ...
        if extra_exclusion is not None:
            if np.asarray(extra_exclusion).shape != result.shape:
                raise MaskShapeError(
                    "extra_exclusion",
                    result.shape,
                    np.asarray(extra_exclusion).shape,
                )
    ...
    def evidence(self) -> MaskEvidence:
        masks = {
            "valid_skin": self.valid_skin_mask,
            "hair": self.hair_mask,
            "facial_hair": self.facial_hair_mask,
            "feature_exclusion": self.feature_exclusion_mask,
            "nostril": self.nostril_mask,
            "display_face": self.display_face_mask,
        }
        return {
            "mask_contract_version": MASK_CONTRACT_VERSION,
            "coordinate_space": self.coordinate_space,
            "sha256": {name: _mask_sha256(mask) for name, mask in masks.items()},
            "pixels": {
                name: int(cv2.countNonZero(np.asarray(mask)))
                for name, mask in masks.items()
            },
        }


def build_analysis_mask_bundle(
    *,
    valid_skin_mask: np.ndarray,
    hair_mask: np.ndarray,
    facial_hair_mask: np.ndarray,
    ...
    valid = cv2.bitwise_and(
        _binary_readonly(valid_skin_mask, shape),
        cv2.bitwise_not(excluded),
    )
    valid.flags.writeable = False
    return AnalysisMaskBundle(
        valid_skin_mask=valid,
        hair_mask=hair,
        facial_hair_mask=facial_hair,
        feature_exclusion_mask=features,
        nostril_mask=nostrils,
        display_face_mask=_binary_readonly(display_face_mask, shape),
