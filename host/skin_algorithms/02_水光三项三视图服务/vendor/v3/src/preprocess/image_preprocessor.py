...
from src.preprocess.consumer_quality import evaluate_consumer_quality
from typing_extensions import assert_never

from src.utils.io_utils import cv_imread, cv_imwrite
from src.utils.model_loader import load_face_landmarker, load_selfie_segmenter
...
class PreprocessResultV2:
    ...
    # MediaPipe relative Z in aligned pixel-equivalent units. This is a
    # normalized 2.5D proxy, not metric depth.
...
class Config:
    ...
    HAIR_FOREHEAD_RESCUE_LINE_RESPONSE = 0.30
    ...
    # 口周/下巴的面部毛发必须形成有限长度的细长连通结构。此前只要求
    # “暗线响应 + 邻域密度”，圆形红痘、痣和密集红斑也会被误删。
    FACIAL_HAIR_MIN_COMPONENT_AREA = 6
    FACIAL_HAIR_MAX_COMPONENT_AREA = 2000
    FACIAL_HAIR_MIN_ASPECT_RATIO = 2.2
    # 金属面部饰品常表现为“暗色圆环 + 强白色高光”。它不是毛发，但同样
    # 属于外部遮挡，必须继续从有效皮肤中排除；普通红痘/痣没有这种镜面核心。
    FACIAL_OCCLUSION_HIGHLIGHT_VALUE = 0.94
    FACIAL_OCCLUSION_HIGHLIGHT_MAX_SATURATION = 0.30
...
def build_nostril_exclusion_mask(
    analysis_image: np.ndarray,
    points: np.ndarray,
    ...
    radius_x = int(
        np.clip(
            round(0.18 * span),
            Config.NOSTRIL_MIN_RADIUS_X,
            Config.NOSTRIL_MAX_RADIUS_X,
        )
    )
    radius_y = int(
        np.clip(
            round(0.11 * span),
            Config.NOSTRIL_MIN_RADIUS_Y,
            Config.NOSTRIL_MAX_RADIUS_Y,
        )
    )
    centres = (
        0.68 * left_outer + 0.32 * nose_base,
        0.68 * right_outer + 0.32 * nose_base,
    )

    lab_l = (
        cv2.cvtColor(analysis_image, cv2.COLOR_BGR2LAB)[:, :, 0]
        if use_photometric_refinement
        else None
    )
    height, width = shape
    for centre_f in centres:
        ...
        cv2.ellipse(
            seed,
            local_centre,
            (max(3, radius_x // 2), max(2, radius_y // 2)),
            0,
            0,
            360,
            1,
...
def build_feature_exclusion_masks(
    analysis_image: np.ndarray,
    points: np.ndarray,
    use_photometric_nostrils: bool = True,
    eyebrow_radius_px: int | None = None,
) -> dict[str, np.ndarray]:
    ...
    eyebrow_core=np.zeros_like(exclusions)
    _fill_landmark_feature(eyebrow_core,points,LEFT_EYEBROW,1)
    _fill_landmark_feature(eyebrow_core,points,RIGHT_EYEBROW,1)
    if eyebrow_radius_px is not None and (not isinstance(eyebrow_radius_px,int) or not 0<=eyebrow_radius_px<=60):
        raise ValueError("invalid eyebrow radius in analysis pixels")
    eyebrow_kernel=Config.EYEBROW_DILATE if eyebrow_radius_px is None else 2*eyebrow_radius_px+1
    eyebrow_exclusion=cv2.dilate(eyebrow_core,cv2.getStructuringElement(cv2.MORPH_ELLIPSE,(eyebrow_kernel,eyebrow_kernel)))
    exclusions=cv2.bitwise_or(exclusions,eyebrow_exclusion)
    _fill_landmark_feature(exclusions, points, LIPS, Config.FEATURE_DILATE)
    nostril_mask = build_nostril_exclusion_mask(
        analysis_image,
        points,
        use_photometric_refinement=use_photometric_nostrils,
    )
    exclusions = cv2.bitwise_or(exclusions, nostril_mask)
    return {
        "feature_exclusions": exclusions,
        "nostril_mask": nostril_mask,
        "eyebrow_core_mask":eyebrow_core,
        "eyebrow_exclusion_mask":eyebrow_exclusion,
    }
...
def _masked_gaussian(
    ...
    result = numerator / np.maximum(denominator, 1e-5)
    result[mask_f == 0] = 0.0
    return result.astype(np.float32)


def _robust_positive_z(
    values: np.ndarray,
    mask: np.ndarray,
    sigma_floor: float,
) -> np.ndarray:
...
def _plausible_strand_component(
    area: float,
    aspect_ratio: float,
    ...
    maximum = max(
        2000.0,
        float(domain_pixels) * Config.HAIR_STRAND_MAX_DOMAIN_RATIO,
    )
    if area > maximum:
        return False
    return bool(
        touches_boundary
        or (in_forehead and aspect_ratio >= 2.2)
        or (aspect_ratio >= 3.0 and area <= 2000.0)
    )


def detect_visible_hair_masks(
    analysis_image: np.ndarray,
    valid_mask: np.ndarray,
    points: np.ndarray,
    feature_exclusions: np.ndarray | None = None,
) -> dict[str, np.ndarray]:
    ...
    if np.count_nonzero(analysis_domain) < 100:
        ...
        return {
            "bulk_hair_mask": empty.copy(),
            "color_hair_mask": empty.copy(),
            "strand_hair_mask": empty.copy(),
            "facial_hair_mask": empty.copy(),
            "hair_mask": empty.copy(),
            "hair_line_response": empty_f.copy(),
            "hair_scale_vote": empty.copy(),
        }
    ...
    # Build a robust central-face skin reference.  Unlike the local dark-line
    # score, this fills broad brown/black bangs whose interior is locally flat.
    ...
    reference_mask = (
        ...
        & (yy > brow_y + 60)
        & (yy < mouth_y - 30)
        & (np.abs(xx - centre_x) < 0.42 * face_width)
    )
    ...
    colour_hair_raw = (
        (
            l_dark
            > max(
                Config.HAIR_COLOR_MIN_L_DELTA,
                Config.HAIR_COLOR_L_MAD * 1.4826 * l_mad,
            )
        )
        | (
            (delta_e_to_skin > delta_e_median + Config.HAIR_COLOR_DE_MAD * 1.4826 * delta_e_mad)
            & (l_dark > Config.HAIR_COLOR_MIN_L_FOR_DELTA_E)
        )
    ) & (analysis_domain > 0) & (yy < brow_y + 30)
    colour_hair_raw = colour_hair_raw.astype(np.uint8) * 255
    colour_hair_raw = cv2.morphologyEx(
        colour_hair_raw,
        cv2.MORPH_CLOSE,
    ...
    for sigma in Config.HAIR_LOCAL_SIGMAS:
        ...
        max_dark_residual = np.maximum(
            max_dark_residual,
            np.maximum(background - l_float, 0.0),
        )
    ...
    strand_raw = (
        (dark_z >= Config.HAIR_DARK_Z)
        & (line_vote >= Config.HAIR_MIN_SCALE_VOTES)
        & (analysis_domain > 0)
    ).astype(np.uint8) * 255
    strand_raw = cv2.morphologyEx(
        strand_raw,
        cv2.MORPH_CLOSE,
        cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3)),
    )
    ...
    cv2.rectangle(
        forehead_zone,
        (0, 0),
        (domain.shape[1] - 1, int(np.clip(brow_y + 20, 0, domain.shape[0] - 1))),
        255,
        -1,
    )

    relaxed_forehead_strand = (
        (max_line_z / 8.0 >= Config.HAIR_FOREHEAD_RESCUE_LINE_RESPONSE)
        & (line_vote >= Config.HAIR_FOREHEAD_RESCUE_MIN_VOTES)
        & (analysis_domain > 0)
        & (forehead_zone > 0)
    ).astype(np.uint8) * 255
    relaxed_forehead_strand = cv2.morphologyEx(
        relaxed_forehead_strand,
        cv2.MORPH_CLOSE,
        cv2.getStructuringElement(
            cv2.MORPH_ELLIPSE,
            (
                Config.HAIR_FOREHEAD_RESCUE_CLOSE_KERNEL,
                Config.HAIR_FOREHEAD_RESCUE_CLOSE_KERNEL,
            ),
        ),
    )
    ...
    for contour in contours:
        ...
        keep = _plausible_strand_component(
            area,
            aspect_ratio,
            touches_boundary,
            in_forehead,
    ...
    bulk_raw = (
        (dark_z >= Config.HAIR_BULK_Z)
        & (analysis_domain > 0)
    ).astype(np.uint8) * 255
    bulk_raw = cv2.morphologyEx(
        bulk_raw,
        cv2.MORPH_CLOSE,
        cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5)),
    )
    ...
    # Colour-based components must either reach the face boundary or overlap a
    # detected strand.  This prevents a single dark spot on clear forehead skin
    # from being promoted to a broad hair exclusion.
    ...
    if len(points) > 17:
        ...
        cv2.rectangle(
            lower_zone,
            (0, nose_bottom_y),
            (domain.shape[1] - 1, domain.shape[0] - 1),
            255,
            -1,
        )
    visible_dark_line = (
        (dark_z >= Config.FACIAL_HAIR_DARK_Z)
        & (line_vote >= Config.HAIR_MIN_SCALE_VOTES)
        & (analysis_domain > 0)
    ).astype(np.uint8)
    density = cv2.boxFilter(
        visible_dark_line.astype(np.float32),
        cv2.CV_32F,
        (21, 21),
        normalize=True,
    )
    facial_hair_raw = (
        (visible_dark_line > 0)
        & (density >= Config.FACIAL_HAIR_DENSITY)
        & (lower_zone > 0)
    ).astype(np.uint8) * 255

    # The photometric response alone cannot distinguish short facial hair
    # from a compact dark/red skin finding.  Keep only genuinely elongated,
    # bounded components.  Broad connected fields are also rejected: they are
    # shadows/skin texture rather than a trustworthy hair occlusion and would
    # otherwise carve large holes from every downstream algorithm's skin mask.
    component_count, component_labels, component_stats, _ = (
        cv2.connectedComponentsWithStats(facial_hair_raw, connectivity=8)
    )
    facial_hair = np.zeros_like(domain)
    hsv = cv2.cvtColor(analysis_image, cv2.COLOR_BGR2HSV).astype(np.float32)
    specular = (
        (hsv[:, :, 2] / 255.0 >= Config.FACIAL_OCCLUSION_HIGHLIGHT_VALUE)
        & (
            hsv[:, :, 1] / 255.0
            <= Config.FACIAL_OCCLUSION_HIGHLIGHT_MAX_SATURATION
        )
    )
    highlight_search_kernel = cv2.getStructuringElement(
        cv2.MORPH_ELLIPSE,
        (
            Config.FACIAL_OCCLUSION_HIGHLIGHT_SEARCH_KERNEL,
            Config.FACIAL_OCCLUSION_HIGHLIGHT_SEARCH_KERNEL,
        ),
    )
    for label in range(1, component_count):
        ...
        if not (
            Config.FACIAL_HAIR_MIN_COMPONENT_AREA
            <= area
            <= Config.FACIAL_HAIR_MAX_COMPONENT_AREA
        ):
            continue
        ...
        highlight_near_component = bool(
            np.any(
                (cv2.dilate(component, highlight_search_kernel) > 0)
                & specular
            )
        )
        if (
            aspect_ratio < Config.FACIAL_HAIR_MIN_ASPECT_RATIO
            and not highlight_near_component
        ):
            continue
    ...
    combined = cv2.dilate(
        ...
        cv2.getStructuringElement(
            cv2.MORPH_ELLIPSE,
            (Config.HAIR_MASK_DILATE, Config.HAIR_MASK_DILATE),
        ),
    )
    combined = cv2.bitwise_and(combined, domain)
    combined = cv2.bitwise_and(combined, cv2.bitwise_not(feature_exclusions))
    return {
        "bulk_hair_mask": bulk_hair,
        "color_hair_mask": colour_hair,
        "strand_hair_mask": strand_hair,
        "facial_hair_mask": facial_hair,
        "hair_mask": combined,
        "hair_line_response": np.clip(max_line_z / 8.0, 0.0, 1.0).astype(np.float32),
        "hair_scale_vote": line_vote,
    }
...
class ImagePreprocessor:
    ...
    def __init__(
        ...
        # Accepted only for source compatibility. V2 never reads or uses it.
        self.legacy_ref_image_path = ref_image_path
        self.capture_profile = capture_profile or capture_profile_from_environment()

        # Two faces are requested so that multiple-person input can be rejected.
    ...
    def _status_from_score(score: float, flags: list[str]) -> tuple[float, str]:
        ...
        if (
            "PARTIAL_FACE" in flags
            and score >= 45.0
            and not severe_capture_failures.intersection(flags)
        ):
    ...
    def _fallback_result(self, img: np.ndarray, flags: list[str]) -> PreprocessResultV2:
        ...
        result = PreprocessResultV2(
            ...
            display_image=analysis.copy(),
            skin_mask=np.zeros((size, size), dtype=np.uint8),
        ...
        self.last_debug_masks = {
            "semantic_skin_mask": empty_mask.copy(),
            "face_geometry_mask": empty_mask.copy(),
            "forehead_completion": empty_mask.copy(),
            "feature_exclusions": empty_mask.copy(),
            "nostril_mask": empty_mask.copy(),
            "bulk_hair_mask": empty_mask.copy(),
            "color_hair_mask": empty_mask.copy(),
            "strand_hair_mask": empty_mask.copy(),
            "facial_hair_mask": empty_mask.copy(),
            "hair_mask": empty_mask.copy(),
            "hair_line_response": empty_mask.astype(np.float32),
            "hair_scale_vote": empty_mask.copy(),
            "display_face_mask": empty_mask.copy(),
        }
    ...
    def _attach_mask_bundle(self, result: PreprocessResultV2) -> None:
        result._debug_masks = {
            key: value.copy()
            for key, value in self.last_debug_masks.items()
        }
        ...
        match profile:
            ...
            case CaptureProfile.CONSUMER:
                if os.environ.get("SHUIGUANG_PREPROCESS_MODE") not in {"semantic_model_only_v2","bisenet_rgb_diagnostic_v1"}:
                    hair, facial_hair = expand_common_hair_masks(hair,facial_hair)
            case unreachable:
                assert_never(unreachable)
        result.mask_bundle = build_analysis_mask_bundle(
            valid_skin_mask=result.skin_mask,
            hair_mask=hair,
            facial_hair_mask=facial_hair,
            feature_exclusion_mask=result._debug_masks.get("feature_exclusions", empty),
            nostril_mask=result._debug_masks.get("nostril_mask", empty),
            display_face_mask=result._debug_masks.get(
                "display_face_mask",
                result.skin_mask,
            ),
            coordinate_space="aligned_1024_dermavision",
        )
    ...
    def _similarity_transform(
        ...
        original_person_mask: np.ndarray | None = None,
    ) -> np.ndarray | None:
        ...
        geometry_mask, forehead_completion = self._forehead_geometry(
            (height, width),
            points,
            base_face_mask,
        )
        ...
        semantic_face = cv2.bitwise_or(
            (original_person_mask > 0).astype(np.uint8) * 255,
            forehead_completion,
        )
        ...
        return np.array(
            [
                [scale, 0.0, scale * (off_x - crop_x_min)],
                [0.0, scale, scale * (off_y - crop_y_min)],
            ],
            dtype=np.float32,
        )
    ...
    def _quality_assessment(
        ...
        score = 20.0  # A valid primary face was detected.

        if face_area_ratio < Config.MIN_FACE_AREA_RATIO:
            flags.append("FACE_TOO_SMALL")
            area_score = 15.0 * np.clip(
                face_area_ratio / Config.MIN_FACE_AREA_RATIO,
                0.0,
                1.0,
            )
            ...
            area_score = 10.0 + 5.0 * (
                (face_area_ratio - Config.MIN_FACE_AREA_RATIO)
                / (Config.IDEAL_FACE_AREA_RATIO - Config.MIN_FACE_AREA_RATIO)
            )
            ...
            area_score = 15.0 * np.clip(
                (1.0 - face_area_ratio) / (1.0 - Config.MAX_FACE_AREA_RATIO),
                0.0,
                1.0,
            )
        ...
        blur_score = 15.0 * np.clip(
            blur_variance / Config.BLUR_GOOD_VARIANCE,
            0.0,
            1.0,
        )
        ...
        exposure_score = 15.0 * (
            1.0 - np.clip(clipped_ratio / Config.MAX_CLIPPED_RATIO, 0.0, 1.0)
        )
        if dark_ratio > 0.12:
        ...
        highlight_score = 10.0 * (
            1.0 - np.clip(highlight_ratio / Config.MAX_HIGHLIGHT_RATIO, 0.0, 1.0)
        )
        ...
        pose_fraction = max(
            abs(yaw) / Config.POSE_REJECT_YAW,
            abs(pitch) / Config.POSE_REJECT_PITCH,
            abs(roll) / Config.POSE_REJECT_ROLL,
        )
        ...
        if (
            abs(yaw) > Config.POSE_REJECT_YAW
            or abs(pitch) > Config.POSE_REJECT_PITCH
            or abs(roll) > Config.POSE_REJECT_ROLL
        ):
    ...
    @staticmethod
    def _forehead_geometry(
        shape: tuple[int, int],
        points: np.ndarray,
        base_face_mask: np.ndarray,
        ...
        upper_y = float(
            np.clip(
                top[1] - Config.FOREHEAD_EXTENSION_RATIO * forehead_height,
                0.0,
                height - 1.0,
            )
        )
        ...
        upper_left = np.array(
            [np.clip(centre_x - half_width, 0.0, width - 1.0), upper_y],
            dtype=np.float32,
        )
        ...
        expanded_base = cv2.dilate(
            base_face_mask,
            cv2.getStructuringElement(
                cv2.MORPH_ELLIPSE,
                (Config.FACE_GEOMETRY_DILATE, Config.FACE_GEOMETRY_DILATE),
            ),
        )
        geometry_mask = cv2.bitwise_or(expanded_base, forehead_mask)
        completion = cv2.bitwise_and(
            forehead_mask,
            cv2.bitwise_not(base_face_mask),
        )
    ...
    def _hair_mask(
        ...
        # Do not open/erode here: thin flyaway hairs are exactly the structures
        # that must stay connected to the upper boundary seed.
        dark = cv2.morphologyEx(
            dark,
            cv2.MORPH_CLOSE,
            cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5)),
        ...
        # Only boundary-connected dark components are hair candidates.  The V2
        # implementation previously seeded the entire upper face, which could
        # incorrectly remove real dark forehead findings.
        boundary = cv2.morphologyEx(
            face_mask,
            cv2.MORPH_GRADIENT,
            cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (11, 11)),
    ...
    def _skin_mask(
        ...
        if os.environ.get("SHUIGUANG_PREPROCESS_MODE")=="bisenet_rgb_diagnostic_v1":
            from pathlib import Path
            from src.preprocess.face_parser import FaceParser
            feature_masks=build_feature_exclusion_masks(analysis_image,points,eyebrow_radius_px=30)
            if not hasattr(self,"face_parser"):
                asset=os.environ.get("SHUIGUANG_FACE_PARSER_MODEL",os.path.join(PROJECT_ROOT,"models","face_parsing_resnet18.onnx"))
                self.face_parser=FaceParser(asset,Path(PROJECT_ROOT).resolve().parents[1])
            skin,debug=self.face_parser.build(analysis_image,feature_masks)
            self.last_debug_masks=debug
            return skin,debug["display_face_mask"]
        size = Config.ANALYSIS_SIZE
        shape = (size, size)
        ...
        # The geometric forehead completion is a deliberate fallback for the
        # exact area that person segmentation commonly under-covers.
        semantic_face_skin = cv2.bitwise_or(
            semantic_skin_mask,
            forehead_completion,
        )
        face_candidate = cv2.bitwise_and(semantic_face_skin, geometry_mask)

        candidate_mode=os.environ.get("SHUIGUANG_PREPROCESS_MODE")=="semantic_model_only_v2"
        feature_masks = build_feature_exclusion_masks(analysis_image, points,eyebrow_radius_px=30 if candidate_mode else None)
        ...
        if candidate_mode:
            ...
            if not os.path.isfile(model_path):
                raise RuntimeError("包内语义毛发模型缺失；禁止下载或回退")
            if not hasattr(self,"semantic_hair_segmenter"):
                self.semantic_hair_segmenter=vision.ImageSegmenter.create_from_options(
                    vision.ImageSegmenterOptions(base_options=mp_python.BaseOptions(model_asset_path=model_path),
                        output_category_mask=True,output_confidence_masks=True))
            skin,debug=build(analysis_image,face_candidate,feature_exclusions,self.semantic_hair_segmenter,points)
            self.last_debug_masks={**debug,"face_geometry_mask":geometry_mask,
                "forehead_completion":forehead_completion,"feature_exclusions":feature_exclusions,
                "nostril_mask":feature_masks["nostril_mask"],
                "eyebrow_core_mask":feature_masks["eyebrow_core_mask"],
                "eyebrow_exclusion_mask":feature_masks["eyebrow_exclusion_mask"]}
            return skin,debug["display_face_mask"]
        ...
        # Opening can move a one-pixel boundary; enforce the exclusions and
        # geometry again so tests and downstream engines have a strict mask.
        skin = cv2.bitwise_and(
            skin,
            cv2.bitwise_not(exclusions),
        )
        skin = cv2.bitwise_and(skin, geometry_mask)

        display_face_mask = cv2.bitwise_and(
            face_candidate,
            cv2.bitwise_not(hair),
        )
        self.last_debug_masks = {
            "semantic_skin_mask": semantic_skin_mask,
            "face_geometry_mask": geometry_mask,
            "forehead_completion": forehead_completion,
            "feature_exclusions": feature_exclusions,
            "nostril_mask": feature_masks["nostril_mask"],
            "bulk_hair_mask": bulk_hair,
            "color_hair_mask": hair_maps["color_hair_mask"],
            "strand_hair_mask": hair_maps["strand_hair_mask"],
            "facial_hair_mask": hair_maps["facial_hair_mask"],
            "hair_mask": hair,
            "hair_line_response": hair_maps["hair_line_response"],
            "hair_scale_vote": hair_maps["hair_scale_vote"],
            "display_face_mask": display_face_mask,
        }
    ...
    def preprocess_image(self, img: np.ndarray) -> PreprocessResultV2:
        ...
        primary_index = max(
            range(len(all_points)),
            key=lambda index: cv2.contourArea(
                cv2.convexHull(all_points[index].astype(np.float32))
            ),
        )
        primary_points = all_points[primary_index]
        primary_relative_z = np.asarray(
            [landmark.z * width for landmark in faces[primary_index]],
            dtype=np.float32,
        )

        quality_score, quality_status, quality_flags = self._quality_assessment(
            img,
            primary_points,
            len(faces),
        )
        ...
        matrix = self._similarity_transform(
            primary_points,
            img.shape[:2],
        ...
        size = Config.ANALYSIS_SIZE
        # This is the analysis image: original sRGB pixels plus square crop/resize only.
        analysis_image = cv2.warpAffine(
            img,
            matrix,
            (size, size),
            flags=cv2.INTER_LINEAR,
        ...
        skin_mask, display_face_mask = self._skin_mask(
            analysis_image,
            aligned_points,
            person_mask,
        )
        ...
        match capture_profile:
            ...
            case CaptureProfile.CONSUMER:
                decision = evaluate_consumer_quality(
                    analysis_image,
                    skin_mask,
                    self.last_debug_masks,
                    quality_score,
                ...
                quality_metrics = {
                    "local_lighting_block_range": decision.local_lighting_block_range,
                    "local_lighting_gradient_p99": decision.local_lighting_gradient_p99,
                    "local_lighting_bright_ratio": decision.local_lighting_bright_ratio,
                }
            case unreachable:
                assert_never(unreachable)

        # Multiple-person inputs remain ambiguous. A single detected side face
        # is analyzable and therefore keeps its visible-skin mask.
        if "MULTIPLE_FACES" in quality_flags:
            skin_mask.fill(0)

        # Human-facing display must not inherit algorithm exclusion masks.
        ...
        result = PreprocessResultV2(
            ...
            face_transform_matrix=matrix.astype(np.float32),
            inverse_transform_matrix=inverse,
            quality_score=quality_score,
            quality_status=quality_status,
            quality_flags=quality_flags,
            quality_metrics=quality_metrics,
            landmarks_relative_z=aligned_relative_z.astype(np.float32),
        )
        self._attach_mask_bundle(result)
        self.last_result = result
        return result
    # ------------------------------------------------------------------
    # Compatibility adapters used by the unchanged V1 Pipeline.
    # ------------------------------------------------------------------
    ...
    def process_batch(self) -> list[str]:
        """Process a directory and save display and analysis products separately."""
        ...
        if not valid_files:
            print(f"⚠️ 输入目录 {self.input_dir} 为空！")
            return []

        print(f"🚀 开始执行 Preprocessor V2，共 {len(valid_files)} 张图...")
        processed_files: list[str] = []
        for filename in sorted(valid_files):
            img_path = os.path.join(self.input_dir, filename)
            img = cv_imread(img_path)
            if img is None:
                print(f"⚠️ 读取失败: {filename}")
                continue

            result = self.preprocess_image(img)
            flags_text = ",".join(result.quality_flags) if result.quality_flags else "NONE"
            print(
                f"   [Quality] {filename}: {result.quality_score:.2f} "
                f"{result.quality_status} [{flags_text}]"
            )

            base_name = os.path.splitext(filename)[0]
            display_name = f"{base_name}_01_Preprocessed.jpg"
            analysis_name = f"{base_name}_01_AnalysisInput.jpg"
            cv_imwrite(os.path.join(self.output_dir, display_name), result.display_image)
            cv_imwrite(os.path.join(self.output_dir, analysis_name), result.analysis_image)
            print(f"✅ V2 展示图完成: {filename} -> {display_name}")

            if result.quality_status == "REJECT":
                print(f"⚠️ 质量不合格，跳过算法输入: {filename}")
                continue

            processed_files.append(analysis_name)
...
if __name__ == "__main__":
    raw_dir = os.path.join(PROJECT_ROOT, "data", "raw_images")
    output_dir = os.path.join(PROJECT_ROOT, "output", "preprocessed")
    processor = ImagePreprocessor(raw_dir, output_dir)
    try:
