...
from sg_core.engines.vascular_structure_engine import LEFT_EYE, LEFT_EYEBROW, LIPS, RIGHT_EYE, RIGHT_EYEBROW, VascularStructureAnalyzer
from sg_core.preprocess.analysis_mask_bundle import COMMON_HAIR_SAFETY_MARGIN_PX
...
class ClinicVascularStructureAnalyzer:
    ...
    EXTREME_SATURATION_AREA_RATIO = 0.1
    MIN_VALID_FACE_RATIO = 0.18

    @classmethod
    def _valid_analysis_mask(cls, face: np.ndarray, exclusion: np.ndarray) -> np.ndarray:
    ...
    def detect(self, cp: Any, *, red_display: np.ndarray | None=None) -> ClinicVascularResult:
        # Specialized from the exact V2 consumer invocation: one RGB image only.
        rgb, pp = cp, None
        ...
        valid = cv2.bitwise_and(self._valid_analysis_mask(face, exclusion), cp.skin_mask)
        valid_pixels = int(np.count_nonzero(valid))
        face_pixels = int(np.count_nonzero(face))
        if valid_pixels < max(20000, int(face_pixels * self.MIN_VALID_FACE_RATIO)):
            return self._empty(image, face, exclusion, valid, red_display, 'INSUFFICIENT_GEOMETRIC_FACE_DOMAIN')
        consumer_rgb_proxy = rgb is cp
        if consumer_rgb_proxy and self._has_extreme_chroma_occlusion(image, valid):
            return self._empty(image, face, exclusion, valid, red_display, 'EXTREME_CHROMA_OCCLUSION')
        ...
        hemoglobin = np.clip(0.45 * self._robust_z(local1, valid) + 0.3 * self._robust_z(local2, valid) + 0.25 * self._robust_z(broad1, valid), 0.0, 8.0).astype(np.float32)
        ...
        if red_aligned is not None:
            ...
            red_line = frangi(self._robust_unit(red_dark, valid, 35.0, 99.7), sigmas=(1, 2, 3, 4, 5), black_ridges=False)
        ...
        colour_support = (colour_z >= 0.45) | (hemoglobin >= 2.0) & (colour_z >= 0.1)
        ...
        candidate = (vesselness >= line_threshold) & (hemoglobin >= 1.0) & colour_support & ~surface_reject & (valid > 0)
        eyelash_guard = self._landmark_zone(cp.landmarks, image.shape[:2], (LEFT_EYE, RIGHT_EYE), self.EYE_CANDIDATE_GUARD_PX)
        candidate &= eyelash_guard == 0
        rgb_visible: np.ndarray | None = None
        if consumer_rgb_proxy:
            lab_a = cv2.cvtColor(image, cv2.COLOR_BGR2LAB)[:, :, 1].astype(np.float32)
            local_red = np.maximum(lab_a - gaussian_filter(lab_a, 7.0), 0.0)
            rgb_visible = self._robust_z(local_red, valid) >= 0.7
            rgb_visible = self._dilate(rgb_visible.astype(np.uint8) * 255, 2) > 0
        ...
        (mask, instances) = self._filter_components(candidate_u8, hemoglobin, vesselness, cp_line, red_line, valid, face_width)
        ...
        if rgb_visible is not None:
            rgb_visibility = ((skeleton > 0) & rgb_visible).astype(np.uint8) * 255
            ...
            rgb_support_ratio = float(np.count_nonzero(rgb_visibility) / max(np.count_nonzero(skeleton), 1))
        metrics = self._metrics(face, exclusion, valid, mask, skeleton, branch, hemoglobin, instances, rgb_support_ratio)
        base = red_aligned if red_aligned is not None else image
        overlay = self.render_overlay(base, skeleton, branch)
        return ClinicVascularResult(face_mask=face, exclusion_mask=exclusion, valid_mask=valid, hemoglobin_response=hemoglobin, vesselness=vesselness, vascular_mask=mask, skeleton_mask=skeleton, branch_mask=branch, rgb_visibility_mask=rgb_visibility, overlay=overlay, metrics=metrics, instances=tuple(instances))
    ...
    @staticmethod
    def _landmark_zone(points: np.ndarray, shape: tuple[int, int], groups: tuple[list[int], ...], radius: int) -> np.ndarray:
    ...
    def _clinic_exclusion(self, image: np.ndarray, face: np.ndarray, cp: Any) -> np.ndarray:
        ...
        for key in ('hair_mask', 'facial_hair_mask'):
            ...
            if isinstance(value, np.ndarray):
                exclusion = cv2.bitwise_or(exclusion, self._dilate(self._binary(value), self.HAIR_GUARD_PX))
        eye_hulls: list[np.ndarray] = []
        brow_points = np.concatenate((points[np.asarray(LEFT_EYEBROW, dtype=np.int32), :2], points[np.asarray(RIGHT_EYEBROW, dtype=np.int32), :2]), axis=0)
        if brow_points.size:
            forehead_end = int(np.clip(round(float(np.max(brow_points[:, 1]))) + 12, 0, face.shape[0]))
            exclusion[:forehead_end] = 255
        for (indices, radius) in ((LEFT_EYE, self.EYE_GUARD_PX), (RIGHT_EYE, self.EYE_GUARD_PX), (LEFT_EYEBROW, self.EYEBROW_GUARD_PX), (RIGHT_EYEBROW, self.EYEBROW_GUARD_PX), (LIPS, self.LIP_GUARD_PX)):
    ...
    def _filter_components(self, candidate: np.ndarray, hemoglobin: np.ndarray, vesselness: np.ndarray, cp_line: np.ndarray, red_line: np.ndarray, valid: np.ndarray, face_width: int) -> tuple[np.ndarray, list[dict[str, Any]]]:
        ...
        for label in range(1, count):
            ...
            eigenvalues = np.sort(np.maximum(np.linalg.eigvalsh(covariance), 1e-06))
            ...
            high_confidence = mean_cp_line >= 0.5 and mean_red_line >= 0.35 and (mean_hemo >= 1.8)
            medium_confidence = mean_cp_line >= 0.68 and mean_hemo >= 2.1
            if not (high_confidence or medium_confidence):
                continue
            if length < 10 and (not (mean_cp_line >= 0.75 and mean_red_line >= 0.65 and (mean_hemo >= 2.5))):
                continue
            ...
            if len(endpoint_points) >= 2:
                span = max((float(np.linalg.norm(endpoint_points[i] - endpoint_points[j])) for i in range(len(endpoint_points)) for j in range(i + 1, len(endpoint_points))))
            else:
                span = float(max(w, h))
            ...
            rows.append({'id': len(rows) + 1, 'bbox': [x, y, w, h], 'centroid': [round(cx, 2), round(cy, 2)], 'region': region, 'confidence': confidence, 'area_px': area, 'skeleton_length_px': length, 'p50_width_px': round(float(np.percentile(widths, 50)), 4) if widths.size else 0.0, 'p90_width_px': round(p90_width, 4), 'p50_redness': round(float(np.percentile(response, 50)), 4), 'p90_redness': round(float(np.percentile(response, 90)), 4), 'mean_cp_line_response': round(mean_cp_line, 6), 'mean_red_line_response': round(mean_red_line, 6), 'branch_points': branch_count, 'tortuosity': round(tortuosity, 4), 'radial_like': radial})
        return (output, rows)

    def _metrics(self, face: np.ndarray, exclusion: np.ndarray, valid: np.ndarray, mask: np.ndarray, skeleton: np.ndarray, branch: np.ndarray, hemoglobin: np.ndarray, instances: list[dict[str, Any]], rgb_support_ratio: float | None) -> dict[str, Any]:
        valid_pixels = int(np.count_nonzero(valid))
        ...
        network_length = sum((int(row['skeleton_length_px']) for row in instances if int(row['branch_points']) > 0))
        ...
        outer_guard = cv2.subtract(self._binary(face), self._erode(face, self.OUTER_FACE_GUARD_PX))
        inside_outer_guard = int(np.count_nonzero((mask > 0) & (outer_guard > 0)))
        return {'algorithm': self.ALGORITHM, 'measurement_channel': 'CP_M', 'red_role': 'CP_DERIVED_PROPOSAL_AND_DISPLAY_NOT_INDEPENDENT_EVIDENCE', 'input_width': 1024, 'input_height': 1024, 'face_pixels': int(np.count_nonzero(face)), 'valid_face_pixels': valid_pixels, 'exclusion_pixels': int(np.count_nonzero(exclusion)), 'vascular_count': len(instances), 'vascular_area_px': area, 'vascular_area_ratio': float(area / max(valid_pixels, 1)), 'vascular_total_length_px': float(length), 'vascular_line_density_per_10k_face_px': float(length * 10000.0 / max(valid_pixels, 1)), 'p50_width_px': float(np.percentile(widths, 50)) if widths.size else 0.0, 'p90_width_px': float(np.percentile(widths, 90)) if widths.size else 0.0, 'p50_redness': float(np.percentile(redness, 50)) if redness.size else 0.0, 'p90_redness': float(np.percentile(redness, 90)) if redness.size else 0.0, 'branch_point_count': int(np.count_nonzero(branch)), 'branch_point_density_per_10k_face_px': float(np.count_nonzero(branch) * 10000.0 / max(valid_pixels, 1)), 'tortuosity_p50': float(np.percentile(tortuosities, 50)) if tortuosities.size else 0.0, 'tortuosity_p90': float(np.percentile(tortuosities, 90)) if tortuosities.size else 0.0, 'network_ratio': float(network_length / max(length, 1)), 'radial_structure_count': sum((bool(row['radial_like']) for row in instances)), 'max_continuous_network_length_px': max((int(row['skeleton_length_px']) for row in instances), default=0), 'cp_rgb_visible_support_ratio': rgb_support_ratio, 'region_distribution': regions, 'left_right_summary': {'left_total_length_px': left_length, 'right_total_length_px': right_length, 'length_asymmetry_ratio': abs(left_length - right_length) / max(left_length + right_length, 1.0), 'left_p90_redness': max(left_red, default=0.0), 'right_p90_redness': max(right_red, default=0.0), 'redness_difference': abs(max(left_red, default=0.0) - max(right_red, default=0.0))}, 'qc_passed': True, 'finding_state': 'DETECTED' if instances else 'NOT_DETECTED', 'self_check': {'outside_valid_pixels': int(np.count_nonzero((mask > 0) & (valid == 0))), 'inside_exclusion_pixels': int(np.count_nonzero((mask > 0) & (exclusion > 0))), 'inside_outer_face_guard_pixels': inside_outer_guard, 'passed': bool(inside_outer_guard == 0 and (not np.any((mask > 0) & ((valid == 0) | (exclusion > 0)))))}, 'limitations': ['仅表示正面二维图像中的血管样线状结构，不代表真实血管数量。', '像素宽度不是实际血管直径，不用于疾病诊断。']}

    def _region_metrics(self, valid: np.ndarray, instances: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
        names = ('forehead', 'nose_alar_nasal_side', 'left_periocular', 'right_periocular', 'left_zygoma', 'right_zygoma', 'left_cheek', 'right_cheek', 'left_jaw', 'right_jaw')
        ...
        left = nx < 0.5
        masks = {'forehead': np.zeros_like(ny, dtype=bool), 'nose_alar_nasal_side': (nx >= 0.38) & (nx <= 0.62) & (ny >= 0.3) & (ny <= 0.68)}
        assigned = masks['forehead'] | masks['nose_alar_nasal_side']
        masks.update({'left_periocular': ~assigned & left & (ny < 0.43), 'right_periocular': ~assigned & ~left & (ny < 0.43), 'left_zygoma': ~assigned & left & (ny >= 0.43) & (ny < 0.55), 'right_zygoma': ~assigned & ~left & (ny >= 0.43) & (ny < 0.55), 'left_cheek': ~assigned & left & (ny >= 0.55) & (ny < 0.73), 'right_cheek': ~assigned & ~left & (ny >= 0.55) & (ny < 0.73), 'left_jaw': ~assigned & left & (ny >= 0.73), 'right_jaw': ~assigned & ~left & (ny >= 0.73)})
    ...
    def _assign_region(x: float, y: float, valid: np.ndarray) -> str:
        ...
        if 0.38 <= nx <= 0.62 and 0.3 <= ny <= 0.68:
    ...
    def _empty(self, image: np.ndarray, face: np.ndarray, exclusion: np.ndarray, valid: np.ndarray, red_display: np.ndarray | None, reason: str) -> ClinicVascularResult:
        zero = np.zeros_like(valid)
        base = self._aligned_optional(red_display, image.shape)
        if base is None:
            base = image
        metrics = {'algorithm': self.ALGORITHM, 'measurement_channel': 'CP_M', 'face_pixels': int(np.count_nonzero(face)), 'valid_face_pixels': int(np.count_nonzero(valid)), 'vascular_count': None, 'qc_passed': False, 'finding_state': 'NOT_ASSESSABLE', 'reason_code': reason}
    ...
    def _robust_unit(values: np.ndarray, mask: np.ndarray, low: float, high: float) -> np.ndarray:
        ...
        output = np.clip((values - lo) / max(hi - lo, 1e-06), 0.0, 1.0).astype(np.float32)
        output[mask == 0] = 0.0
        return output
    ...
    @staticmethod
    def _chromatic_colour_drop(local_r: np.ndarray, local_g: np.ndarray, local_b: np.ndarray) -> np.ndarray:
        """Keep red-selective darkening while rejecting achromatic lines."""
        return np.maximum(0.5 * (np.asarray(local_g) + np.asarray(local_b)) - np.asarray(local_r), 0.0).astype(np.float32)

    @classmethod
    def _has_extreme_chroma_occlusion(cls, image: np.ndarray, valid: np.ndarray) -> bool:
        """Reject heavily colour-painted consumer inputs for vascular scoring."""
        saturation = cv2.cvtColor(np.asarray(image), cv2.COLOR_BGR2HSV)[:, :, 1]
        values = saturation[np.asarray(valid) > 0]
        if values.size < 100:
            return True
        p90 = float(np.percentile(values, 90.0))
        extreme_ratio = float(np.count_nonzero(values >= cls.EXTREME_SATURATION_THRESHOLD) / values.size)
        return bool(p90 >= cls.EXTREME_SATURATION_P90 and extreme_ratio >= cls.EXTREME_SATURATION_AREA_RATIO)
