...
class VascularStructureAnalyzer:
    ...
    def _detect_core(self, image: np.ndarray, face_mask: np.ndarray, valid_mask: np.ndarray, exclusion_mask: np.ndarray, landmarks: Optional[np.ndarray], mode: str) -> VascularResult:
        ...
        log_rg = np.log((r + 0.0001) / (g + 0.0001))
        ...
        candidate = ((line_response >= line_thr) & (redness >= red_thr) | (line_response >= strong_line_thr) & (redness >= max(0.55 * red_thr, self.MIN_REDNESS_Z))) & (valid > 0)
        ...
        self_check = {'mask_binary': bool(binary_ok), 'overlap_with_exclusion_pixels': overlap_exclusion, 'outside_valid_mask_pixels': outside_valid, 'candidate_count_nonnegative': len(instances) >= 0, 'passed': bool(binary_ok and overlap_exclusion == 0 and (outside_valid == 0))}
        metrics = VascularMetrics(algorithm=self.ALGORITHM, mode=mode, input_width=int(image.shape[1]), input_height=int(image.shape[0]), face_pixels=int(np.count_nonzero(face_mask)), valid_skin_pixels=valid_area, exclusion_pixels=int(np.count_nonzero(exclusion_mask)), vascular_count=len(instances), vascular_area_px=vascular_area, vascular_area_ratio=float(vascular_area / max(valid_area, 1)), vascular_total_length_px=float(skel_len), vascular_line_density_per_10k_skin_px=float(skel_len * 10000.0 / max(valid_area, 1)), p50_width_px=float(np.percentile(widths, 50)) if widths.size else 0.0, p90_width_px=float(np.percentile(widths, 90)) if widths.size else 0.0, p50_redness=float(np.percentile(red_skel, 50)) if red_skel.size else 0.0, p90_redness=float(np.percentile(red_skel, 90)) if red_skel.size else 0.0, branch_point_count=branch_count, branch_point_density_per_10k_skin_px=float(branch_count * 10000.0 / max(valid_area, 1)), network_ratio=float(network_skeleton / max(skel_len, 1)), region_distribution=region_distribution, self_check=self_check, limitations=['仅表示白光图像中的红色线状血管样结构候选，不代表真实血管数量。', '像素宽度不是实际血管直径。', '普通RGB受白平衡、曝光、妆容和压缩影响。', 'cloud_fallback结果只用于算法自查；本地正式运行应使用MediaPipe landmarks与Preprocessor V2屏蔽Mask。'])
        overlay = self._overlay(image, face_mask, final_mask, skeleton)
        heatmap = self._heatmap(red_unit, line_response, final_mask)
        self.last_debug = {'face_mask': face_mask, 'exclusion_mask': exclusion_mask, 'valid_mask': valid, 'redness_map': redness, 'line_response': line_response, 'raw_candidate_mask': raw_candidate, 'vascular_mask': final_mask, 'skeleton_mask': skeleton, 'branch_mask': branch_mask}
        return VascularResult(face_mask=face_mask, exclusion_mask=exclusion_mask, valid_mask=valid, redness_map=redness, line_response=line_response, vascular_mask=final_mask, skeleton_mask=skeleton, branch_mask=branch_mask, heatmap=heatmap, overlay=overlay, instances=instances, metrics=metrics)

    def _build_project_exclusion(self, image: np.ndarray, face_mask: np.ndarray, landmarks: np.ndarray, debug: dict[str, Any]) -> np.ndarray:
        shape = face_mask.shape
        exclusion = np.zeros(shape, np.uint8)
        for (key, extra) in (('feature_exclusions', 0), ('nostril_mask', self.NOSTRIL_EXTRA_PX), ('hair_mask', self.HAIR_EXTRA_PX), ('facial_hair_mask', self.FACIAL_HAIR_EXTRA_PX)):
            arr = debug.get(key)
    ...
    def _landmark_nostril_mask(self, image: np.ndarray, points: np.ndarray) -> np.ndarray:
        ...
        for cf in centres:
            ...
            if roi.size:
                ...
                mad = float(np.median(np.abs(roi.astype(np.float32) - med)))
    ...
    def _fallback_exclusion(self, image: np.ndarray, face: np.ndarray) -> np.ndarray:
        ...
        (cx1, cx2) = (int(x0 + 0.3 * fw), int(x0 + 0.7 * fw))
        ...
        cv2.ellipse(e, (cx1, brow_y), (int(0.2 * fw), int(0.065 * fh)), 0, 0, 360, 255, -1)
        cv2.ellipse(e, (cx2, brow_y), (int(0.2 * fw), int(0.065 * fh)), 0, 0, 360, 255, -1)
        mouth_y = int(y0 + 0.73 * fh)
        cv2.ellipse(e, (int((x0 + x1) / 2), mouth_y), (int(0.26 * fw), int(0.1 * fh)), 0, 0, 360, 255, -1)
        ...
        if abs(rel) > 0.035:
            ...
            cv2.ellipse(e, (side_x, int(y0 + 0.3 * fh)), (int(0.25 * fw), int(0.1 * fh)), 0, 0, 360, 255, -1)
        gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
        upper = np.zeros_like(face)
        cv2.rectangle(upper, (x0, y0), (x1, int(y0 + 0.43 * fh)), 255, -1)
    ...
    def _filter_components(self, candidate: np.ndarray, redness: np.ndarray, line_response: np.ndarray, valid: np.ndarray, landmarks: Optional[np.ndarray]) -> tuple[np.ndarray, list[VascularInstance]]:
        ...
        for lab_id in range(1, num):
            ...
            eig = np.sort(np.maximum(np.linalg.eigvalsh(cov), 1e-06))
            ...
            kept[comp] = 255
            instances.append(VascularInstance(id=len(instances) + 1, bbox=[x, y, w, h], centroid=[round(cx, 2), round(cy, 2)], area_px=area, skeleton_length_px=skel_len, mean_width_px=round(mean_width, 3), p90_width_px=round(p90_width, 3), elongation=round(elong, 3), p50_redness=round(p50_red, 4), p90_redness=round(p90_red, 4), mean_line_response=round(mean_line, 6), branch_points=branch_count, region=region))
        return (kept, instances)
    ...
    def _heatmap(red_unit: np.ndarray, line_response: np.ndarray, vascular: np.ndarray) -> np.ndarray:
        score = np.clip(0.55 * red_unit + 0.45 * np.clip(line_response / max(float(line_response.max()), 1e-06), 0, 1), 0, 1)
    ...
    def _empty(self, image, face, exclusion, valid, mode, reason):
        ...
        metrics = VascularMetrics(algorithm=self.ALGORITHM, mode=mode, input_width=image.shape[1], input_height=image.shape[0], face_pixels=int(np.count_nonzero(face)), valid_skin_pixels=int(np.count_nonzero(valid)), exclusion_pixels=int(np.count_nonzero(exclusion)), vascular_count=0, vascular_area_px=0, vascular_area_ratio=0.0, vascular_total_length_px=0.0, vascular_line_density_per_10k_skin_px=0.0, p50_width_px=0.0, p90_width_px=0.0, p50_redness=0.0, p90_redness=0.0, branch_point_count=0, branch_point_density_per_10k_skin_px=0.0, network_ratio=0.0, region_distribution={}, self_check={'passed': True, 'reason': reason}, limitations=['empty result'])
    ...
    def _robust_unit(values: np.ndarray, mask: np.ndarray, low: float, high: float) -> np.ndarray:
        ...
        hi = max(float(hi), float(lo) + 1e-06)
...
def _left_right_summary(region_distribution: dict[str, dict[str, float | int]]) -> dict[str, Any]:
    ...
    summary: dict[str, dict[str, float | int]] = {'left': {'count': 0, 'total_length_px': 0.0, 'total_area_px': 0}, 'right': {'count': 0, 'total_length_px': 0.0, 'total_area_px': 0}}
    ...
    return {**summary, 'length_asymmetry_ratio': abs(left - right) / denominator, 'dominant_side': 'left' if left > right else 'right' if right > left else 'balanced'}

def vascular_candidate_metrics(result: VascularResult) -> dict[str, Any]:
    payload = asdict(result.metrics)
    payload.update({'capability_status': 'candidate', 'evidence_status': 'candidate_algorithm', 'score_status': 'reference_not_ready', 'vascular_line_density': result.metrics.vascular_line_density_per_10k_skin_px, 'branch_point_density': result.metrics.branch_point_density_per_10k_skin_px, 'left_right_summary': _left_right_summary(result.metrics.region_distribution)})
    available = ['vascular_count', 'vascular_area_px', 'vascular_area_ratio', 'vascular_total_length_px', 'vascular_line_density', 'p50_width_px', 'p90_width_px', 'p50_redness', 'p90_redness', 'branch_point_count', 'branch_point_density', 'network_ratio', 'region_distribution', 'left_right_summary']
...
def save_vascular_result(result: VascularResult, output_dir: str | Path, input_name: str) -> dict[str, str]:
    ...
    paths = {'overlay': out / '05_血管样结构检测结果图.jpg', 'heatmap': out / '02_血管样结构热力图.jpg', 'vascular_mask': out / '03_血管样结构Mask.png', 'skeleton': out / '04_血管骨架Mask.png', 'branch': out / '05_分支点Mask.png', 'exclusion': out / '06_屏蔽区域Mask.png', 'valid': out / '07_有效分析区域Mask.png', 'metrics': out / '血管样结构量化指标.json', 'csv': out / '血管样结构量化指标.csv', 'medical_v2_csv': out / '血管样结构医学量化指标_V2.csv', 'instances': out / '血管样结构实例.json'}
    ...
    payload = vascular_candidate_metrics(result)
    payload['input_name'] = input_name
    paths['metrics'].write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding='utf-8')
    scalar_rows = [(key, value, 'engineering_value') for (key, value) in payload.items() if isinstance(value, (int, float)) and (not isinstance(value, bool))]
...
__all__ = ['VascularStructureAnalyzer', 'VascularResult', 'VascularMetrics', 'VascularInstance', 'vascular_candidate_metrics', 'save_vascular_result']
