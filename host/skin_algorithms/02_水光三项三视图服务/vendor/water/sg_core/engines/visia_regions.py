...
from sg_core.preprocess.image_preprocessor import LEFT_EYE, LIPS, RIGHT_EYE, build_feature_exclusion_masks
from sg_core.engines.visia_unified_contour import build_unified_visia_scope, draw_unified_visia_contour
REGION_ORDER = ('forehead', 'left_cheek', 'right_cheek', 'nose', 'chin')
REGION_LABELS = {'forehead': '额头', 'left_cheek': '左脸颊', 'right_cheek': '右脸颊', 'nose': '鼻部', 'chin': '下巴'}
...
NOSE_LANDMARKS = (168, 6, 197, 195, 5, 4, 1, 19, 94, 2, 97, 98, 327, 326)
DISPLAY_NOSE_OUTLINE_LANDMARKS = (168, 193, 122, 196, 3, 51, 45, 220, 115, 48, 64, 98, 97, 2, 326, 327, 294, 278, 344, 440, 275, 281, 248, 419, 351, 417)
...
FACE_OVAL_LANDMARKS = (10, 338, 297, 332, 284, 251, 389, 356, 454, 323, 361, 288, 397, 365, 379, 378, 400, 377, 152, 148, 176, 149, 150, 136, 172, 58, 132, 93, 234, 127, 162, 21, 54, 103, 67, 109)
NASOLABIAL_PATHS = ((209, 203, 206, 61), (429, 423, 426, 291))
...
def _unified_public_boundary_mask(shape: tuple[int, int], contour: np.ndarray | None, separator: np.ndarray | None) -> np.ndarray:
    line = np.zeros(shape, dtype=np.uint8)
    if contour is not None and len(contour) >= 4:
        cv2.polylines(line, [np.asarray(contour, dtype=np.int32)], True, 255, PUBLIC_BOUNDARY_LINE_THICKNESS_PX, lineType=cv2.LINE_8)
    if separator is not None and len(separator) >= 2:
        cv2.polylines(line, [np.asarray(separator, dtype=np.int32)], False, 255, PUBLIC_BOUNDARY_LINE_THICKNESS_PX, lineType=cv2.LINE_8)
    kernel_size = 2 * PUBLIC_BOUNDARY_SAFETY_MARGIN_PX + 1
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (kernel_size, kernel_size))
    return cv2.dilate(line, kernel)

def polygon_mask(shape: tuple[int, int], landmarks: np.ndarray, indices: tuple[int, ...] | list[int]) -> np.ndarray:
...
def build_nasolabial_corridor_mask(shape: tuple[int, int], landmarks: np.ndarray, analysis_mask: np.ndarray, *, radius_px: int=25) -> tuple[np.ndarray, np.ndarray]:
    ...
    for indices in NASOLABIAL_PATHS:
        ...
        cv2.polylines(centerline, [polyline], False, 255, thickness=5, lineType=cv2.LINE_AA)
        cv2.polylines(corridor, [polyline], False, 255, thickness=2 * max(1, int(radius_px)) + 1, lineType=cv2.LINE_AA)
    valid = (analysis_mask > 0).astype(np.uint8) * 255
    return (cv2.bitwise_and(corridor, valid), cv2.bitwise_and(centerline, valid))

def build_nasolabial_shadow_mask(image: np.ndarray, landmarks: np.ndarray, analysis_mask: np.ndarray, *, radius_px: int=25, output_margin_px: int=5, restrict_frangi_to_corridor: bool=False) -> np.ndarray:
    ...
    """
    (corridor, centerline) = build_nasolabial_corridor_mask(analysis_mask.shape, landmarks, analysis_mask, radius_px=radius_px)
    ...
    if restrict_frangi_to_corridor:
        ...
        cropped_response = frangi(filled[y0:y1, x0:x1], sigmas=frangi_sigmas, black_ridges=True).astype(np.float32)
        line_response = np.zeros_like(filled, dtype=np.float32)
        line_response[y0:y1, x0:x1] = cropped_response
    else:
        line_response = frangi(filled, sigmas=frangi_sigmas, black_ridges=True).astype(np.float32)
    line_values = line_response[search]
    high = max(float(np.percentile(line_values, 98.5)), 1e-08)
    line_response = np.clip(line_response / high, 0.0, 1.0)
    candidate = (search & (dark_residual >= dark_threshold) & (line_response >= 0.16)).astype(np.uint8) * 255
    candidate = cv2.morphologyEx(candidate, cv2.MORPH_CLOSE, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (7, 7)))
    path_support = cv2.dilate(centerline, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (21, 21)))
    ...
    if output_margin_px > 0 and np.count_nonzero(shadow):
        ...
        shadow = cv2.dilate(shadow, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (2 * radius + 1, 2 * radius + 1)))
    return cv2.bitwise_and(shadow, (analysis_mask > 0).astype(np.uint8) * 255)

def build_moustache_feature_exclusion_mask(shape: tuple[int, int], landmarks: np.ndarray, analysis_mask: np.ndarray, *, margin_px: int=12) -> np.ndarray:
    ...
    if points.ndim != 2 or points.shape[1] < 2 or any((index < 0 or index >= len(points) for index in required)):
        return mask
    ...
    return cv2.bitwise_and(mask, (np.asarray(analysis_mask) > 0).astype(np.uint8) * 255)
...
def _smooth_binary_mask(mask: np.ndarray, sigma: float=2.0) -> np.ndarray:
    ...
    soft = cv2.GaussianBlur(closed.astype(np.float32), (0, 0), sigmaX=sigma, sigmaY=sigma)
...
def _display_face_envelope(skin_mask: np.ndarray) -> np.ndarray:
    ...
    if not np.any(binary):
        return binary
    closed = cv2.morphologyEx(binary, cv2.MORPH_CLOSE, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (DISPLAY_FACE_CLOSE_KERNEL, DISPLAY_FACE_CLOSE_KERNEL)))
...
def _ordered_face_oval_mask(shape: tuple[int, int], landmarks: np.ndarray) -> np.ndarray:
...
def _interpolated_landmark_curve(landmarks: np.ndarray, indices: tuple[int, ...], width: int, *, y_offset: float=0.0) -> np.ndarray:
    """Return a stable per-column curve through the selected landmarks."""
    points = np.asarray([landmarks[index, :2] for index in indices if 0 <= index < len(landmarks)], dtype=np.float32)
    if len(points) < 2:
    ...
    curve = np.interp(np.arange(width, dtype=np.float32), unique_x, unique_y, left=float(unique_y[0]), right=float(unique_y[-1])).astype(np.float32)
    curve = cv2.GaussianBlur(curve.reshape(1, -1), (0, 0), sigmaX=12.0).ravel()
    return curve

def build_visia_regions(analysis_image: np.ndarray, skin_mask: np.ndarray, landmarks: np.ndarray, quality_flags: list[str] | tuple[str, ...] | None=None, *, include_chin: bool=True, mode: str='full', feature_margin_px: int=10) -> VisiaRegionSet:
    ...
    feature_masks = build_feature_exclusion_masks(analysis_image, landmarks[:, :2].astype(np.float32), use_photometric_nostrils=True)
    exclusions = feature_masks['feature_exclusions']
    if feature_margin_px > 0:
        radius = int(feature_margin_px)
        exclusions = cv2.dilate(exclusions, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (2 * radius + 1, 2 * radius + 1)))
    ...
    centre_x = int(np.clip(landmarks[1, 0] if len(landmarks) > 1 else np.median(valid_x), 0, width - 1))
    brow_y = int(np.clip(min(landmarks[BROW_LANDMARKS[0], 1], landmarks[BROW_LANDMARKS[1], 1]), int(valid_y.min()), int(valid_y.max())))
    mouth_y = int(np.clip(np.mean(landmarks[list(MOUTH_LANDMARKS), 1]), int(valid_y.min()), int(valid_y.max())))
    ...
    display_oval = cv2.dilate(display_oval, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (DISPLAY_FOREHEAD_EXTENSION_KERNEL, DISPLAY_FOREHEAD_EXTENSION_KERNEL)))
    display_safe = _display_face_envelope(cv2.bitwise_and(base_skin, display_oval))
    skin = safe_skin > 0
    display_skin = display_safe > 0
    forehead = ((yy < brow_y) & skin).astype(np.uint8) * 255
    forehead_curve = _interpolated_landmark_curve(landmarks, FOREHEAD_CURVE_LANDMARKS, width, y_offset=-10.0)
    if not np.any(forehead_curve):
        forehead_curve = np.full(width, float(brow_y), dtype=np.float32)
    mouth_curve = _interpolated_landmark_curve(landmarks, MOUTH_CURVE_LANDMARKS, width, y_offset=14.0)
    ...
    display_forehead = ((yy < forehead_curve[None, :]) & display_skin).astype(np.uint8) * 255
    middle = (yy >= brow_y) & (yy <= mouth_y) & skin & (nose == 0)
    display_nose = polygon_mask(skin_mask.shape, landmarks, list(NOSE_LANDMARKS))
    display_nose = cv2.dilate(display_nose, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (31, 31)))
    display_nose = cv2.bitwise_and(display_nose, display_safe)
    display_middle = (yy >= forehead_curve[None, :]) & (yy <= mouth_curve[None, :]) & display_skin & (display_nose == 0)
    left_cheek = (middle & (xx < centre_x)).astype(np.uint8) * 255
    right_cheek = (middle & (xx >= centre_x)).astype(np.uint8) * 255
    display_left_cheek = (display_middle & (xx < centre_x)).astype(np.uint8) * 255
    display_right_cheek = (display_middle & (xx >= centre_x)).astype(np.uint8) * 255
    chin = ((yy > mouth_y) & skin).astype(np.uint8) * 255
    display_chin = ((yy > mouth_curve[None, :]) & display_skin).astype(np.uint8) * 255
    raw_regions = {'forehead': forehead, 'left_cheek': left_cheek, 'right_cheek': right_cheek, 'nose': nose, 'chin': chin}
    raw_display_regions = {'forehead': display_forehead, 'left_cheek': display_left_cheek, 'right_cheek': display_right_cheek, 'nose': display_nose, 'chin': display_chin}
    ...
    for name in ('nose', 'forehead', 'left_cheek', 'right_cheek', 'chin'):
        ...
        display_mask = raw_display_regions[name] if name in enabled else np.zeros_like(safe_skin)
        display_mask = _smooth_binary_mask(display_mask, sigma=3.0)
        display_mask = cv2.bitwise_and(display_mask, display_safe)
        display_mask = cv2.bitwise_and(display_mask, cv2.bitwise_not(display_occupied))
    ...
    (display_contour, scope_mask, display_separator) = build_unified_visia_scope(np.asarray(landmarks, dtype=np.float32), skin_mask.shape, display_safe, partial_face=partial_face, left_visible_area=int(np.count_nonzero(raw_regions['left_cheek'])), right_visible_area=int(np.count_nonzero(raw_regions['right_cheek'])))
    ...
    for name in ('nose', 'forehead', 'left_cheek', 'right_cheek', 'chin'):
        ...
        display_clipped = cv2.bitwise_and(display_clipped, cv2.bitwise_not(display_occupied))
        display_regions[name] = display_clipped
        display_occupied = cv2.bitwise_or(display_occupied, display_clipped)
    public_boundary_safety = _unified_public_boundary_mask(safe_skin.shape, display_contour, display_separator)
    scope_mask = cv2.bitwise_and(scope_mask, cv2.bitwise_not(public_boundary_safety))
    occupied = np.zeros_like(safe_skin)
    for name in ('nose', 'forehead', 'left_cheek', 'right_cheek', 'chin'):
        regions[name] = cv2.bitwise_and(regions[name], scope_mask)
        occupied = cv2.bitwise_or(occupied, regions[name])
    return VisiaRegionSet(analysis_mask=occupied, feature_exclusion_mask=exclusions, regions=regions, display_regions=display_regions, partial_face=partial_face, display_contour=display_contour, display_separator=display_separator, scope_mask=scope_mask, public_boundary_safety_mask=public_boundary_safety)

def masked_gaussian(channel: np.ndarray, mask: np.ndarray, sigma: float) -> np.ndarray:
    ...
    numerator = cv2.GaussianBlur(values * valid, (0, 0), sigmaX=float(sigma), sigmaY=float(sigma), borderType=cv2.BORDER_REFLECT)
    denominator = cv2.GaussianBlur(valid, (0, 0), sigmaX=float(sigma), sigmaY=float(sigma), borderType=cv2.BORDER_REFLECT)
    result = numerator / np.maximum(denominator, 1e-06)
    result[valid == 0] = 0.0
    return result.astype(np.float32)

def regional_positive_z(values: np.ndarray, region_set: VisiaRegionSet, sigma_floor: float=0.01) -> np.ndarray:
    ...
    for mask in region_set.regions.values():
        ...
        output[valid] = np.maximum((values[valid].astype(np.float32) - median) / scale, 0.0)
    output[region_set.analysis_mask == 0] = 0.0
    return np.clip(output, 0.0, 20.0)

def robust_unit_map(values: np.ndarray, mask: np.ndarray, low: float=5.0, high: float=98.0) -> np.ndarray:
    valid = mask > 0
    output = np.zeros_like(values, dtype=np.float32)
    ...
    if hi <= lo + 1e-06:
        return output
    output[valid] = np.clip((values[valid].astype(np.float32) - float(lo)) / float(hi - lo), 0.0, 1.0)
    return output

def assign_region(x: float, y: float, regions: dict[str, np.ndarray]) -> str:
    (px, py) = (int(round(x)), int(round(y)))
    for name in ('nose', 'forehead', 'left_cheek', 'right_cheek', 'chin'):
        mask = regions.get(name)
        if mask is not None and 0 <= py < mask.shape[0] and (0 <= px < mask.shape[1]) and (mask[py, px] > 0):
            return name
    return 'other'

def _chaikin_closed_curve(contour: np.ndarray, iterations: int=DISPLAY_CHAIKIN_ITERATIONS) -> np.ndarray:
...
def _clean_display_mask(mask: np.ndarray) -> np.ndarray:
    ...
    if not np.count_nonzero(binary):
        return binary
    binary = cv2.morphologyEx(binary, cv2.MORPH_CLOSE, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (DISPLAY_CLOSE_KERNEL, DISPLAY_CLOSE_KERNEL)))
    soft = cv2.GaussianBlur(binary, (0, 0), sigmaX=DISPLAY_SMOOTH_SIGMA, sigmaY=DISPLAY_SMOOTH_SIGMA, borderType=cv2.BORDER_REFLECT)
    return (soft >= 127.0).astype(np.uint8) * 255

def _draw_smooth_mask_contours(canvas: np.ndarray, mask: np.ndarray, color: tuple[int, int, int], thickness: int, *, largest_only: bool) -> None:
    (contours, _) = cv2.findContours(_clean_display_mask(mask), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
    contours = [contour for contour in contours if abs(float(cv2.contourArea(contour))) >= DISPLAY_MIN_CONTOUR_AREA]
    if not contours:
        return
    if largest_only:
    ...
    for contour in contours:
        ...
        cv2.polylines(canvas, [smooth], True, color, thickness, lineType=cv2.LINE_AA)

def _draw_smooth_nose_landmark_contour(canvas: np.ndarray, landmarks: np.ndarray, color: tuple[int, int, int], thickness: int) -> bool:
    ...
    """
    points = np.asarray(landmarks, dtype=np.float32)
    valid = [index for index in DISPLAY_NOSE_OUTLINE_LANDMARKS if 0 <= index < len(points)]
...
def draw_region_boundaries(image: np.ndarray, regions: dict[str, np.ndarray], color: tuple[int, int, int]=VISIA_BOUNDARY_COLOR, thickness: int=3, *, partial_face: bool=False, nose_style: str='hidden', landmarks: np.ndarray | None=None, closed_contour: np.ndarray | None=None, separator_contour: np.ndarray | None=None) -> np.ndarray:
    ...
    if closed_contour is not None and len(closed_contour) >= 4:
        return draw_unified_visia_contour(output, closed_contour, color, thickness, separator_contour)
    if partial_face:
        visible = np.zeros(image.shape[:2], dtype=np.uint8)
        for mask in regions.values():
            visible = cv2.bitwise_or(visible, mask)
        _draw_smooth_mask_contours(output, visible, color, thickness, largest_only=True)
        return output
    groups = [regions.get('forehead'), cv2.bitwise_or(regions.get('left_cheek', np.zeros(image.shape[:2], np.uint8)), regions.get('right_cheek', np.zeros(image.shape[:2], np.uint8))), regions.get('chin')]
    for mask in groups:
        if mask is None or np.count_nonzero(mask) < 100:
            continue
        _draw_smooth_mask_contours(output, mask, color, thickness, largest_only=False)
    if nose_style == 'hidden':
        return output
    if nose_style == 'smooth' and landmarks is not None and _draw_smooth_nose_landmark_contour(output, landmarks, color, thickness):
        return output
    nose_mask = regions.get('nose')
    if nose_mask is not None and np.count_nonzero(nose_mask) >= 100:
        _draw_smooth_mask_contours(output, nose_mask, color, thickness, largest_only=False)
    return output

def compact_region_counts(locations: list[dict], partial_face: bool) -> tuple[list[str], list[int]]:
